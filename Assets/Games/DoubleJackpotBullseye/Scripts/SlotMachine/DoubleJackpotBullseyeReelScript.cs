...
public class DoubleJackpotBullseyeReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<DoubleJackpotBullseyeSlotScript> slots;
    private RectTransform _rectTransform;
    private DoubleJackpotBullseyeSpinSettings _spinSettings;
    ...
    // Events
    public delegate void DoubleJackpotBullseyeReelEvents(int index);
    public static event DoubleJackpotBullseyeReelEvents OnSpinStart;
    public static event DoubleJackpotBullseyeReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<DoubleJackpotBullseyeSlotScript>(GetComponentsInChildren<DoubleJackpotBullseyeSlotScript>());
        ...
        _spinSettings = DoubleJackpotBullseyeSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        DoubleJackpotBullseyeSlotMachine.Instance.isResultReceived = false;
    }
    ...
    {
        if (reelIndex >= DoubleJackpotBullseyeSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = DoubleJackpotBullseyeSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = DoubleJackpotBullseyeSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
        ...
        var extraSlot = DoubleJackpotBullseyeSlotMachine.GetResourceById("Empty");

        if (_clampedDown)
        {
            slots[3].SetType(extraSlot.Value);
        }
        else
        {
    ...
    public DoubleJackpotBullseyeSlotType GetSlotType(int index)
    {
    ...
    {
        slots[0].SetType(DoubleJackpotBullseyeSlotMachine.CachedRealSymbols[Random.Range(0, DoubleJackpotBullseyeSlotMachine.CachedRealSymbols.Count)]);
        ...
        if (_spinSettings.endSpin == DoubleJackpotBullseyeSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? DoubleJackpotBullseyeGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? DoubleJackpotBullseyeGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private void OnClampComplete()
    {
        if (_clampedDown)
        {
            ...
            var topSlot = DoubleJackpotBullseyeSlotMachine.GetResourceById("Empty");
    ...
    private DoubleJackpotBullseyeSlotResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = DoubleJackpotBullseyeSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return DoubleJackpotBullseyeSlotMachine.CachedEmptySymbol.Value;
        }
    }
