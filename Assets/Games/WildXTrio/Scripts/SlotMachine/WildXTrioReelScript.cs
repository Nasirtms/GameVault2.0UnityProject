...
using UnityEngine;
public class WildXTrioReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private WildXTrioGameSettings settings;

    [Header("References")]
    public List<WildXTrioSlotScript> slots;
    ...
    // Direction control
    private WildXTrioSpinDirection currentSpinDirection = WildXTrioSpinDirection.Down;
    ...
    {
        settings = WildXTrioSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<WildXTrioSlotScript>(GetComponentsInChildren<WildXTrioSlotScript>());
        }
    ...
    {
        foreach (WildXTrioSlotScript slot in slots)
        {
    ...
    {
        if (reelIndex >= WildXTrioSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = WildXTrioSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = WildXTrioSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                WildXTrioSlotMachine.Instance.isResultReceived = true;
        ...
        var extraSlot = WildXTrioSlotMachine.GetResourceById("Empty");
        if (extraSlot.HasValue)
        {
            bool centerIsEmpty = slots[3].slotType == WildXTrioSlotType.Empty;

            if (!centerIsEmpty)
            {
                slots[0].SetType(
                WildXTrioSlotMachine.CachedRealSymbols[
                    Random.Range(0, WildXTrioSlotMachine.CachedRealSymbols.Count)]);
    ...
    public void SetSpinDirection(WildXTrioSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == WildXTrioSpinDirection.Up)
        {
    ...
    private WildXTrioSlotResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = WildXTrioSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return WildXTrioSlotMachine.CachedEmptySymbol.Value;
        }
    }
