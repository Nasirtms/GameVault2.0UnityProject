...
[RequireComponent(typeof(WildBuffaloBetController))]
[RequireComponent(typeof(WildBuffaloAutoSpinController))]
public class WildBuffaloUIManager : GameBetServices
{
    #region Variables
    public static WildBuffaloUIManager Instance;
    [SerializeField] private WildBuffaloRulesPopupController rulesPopupController;
    ...
    private WildBuffaloBetController betController;
    private WildBuffaloAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<WildBuffaloBetController>();
        autoSpinController = GetComponent<WildBuffaloAutoSpinController>();
    ...
    public void PlaySound(string soundName)
    {
        ...
        if (!WildBuffaloSoundManager.Instance.IsSoundMute())
            WildBuffaloSoundManager.Instance.PlaySFX(soundName);
    }

    public void StopCurrentSFX()
    {
        if (!WildBuffaloSoundManager.Instance.IsSoundMute())
            WildBuffaloSoundManager.Instance.StopSFX();
    }

    public void PlayMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!WildBuffaloSoundManager.Instance.IsMusicMute())
            WildBuffaloSoundManager.Instance.PlayMusic(soundName);
    }
    ...
    public void StopMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!WildBuffaloSoundManager.Instance.IsMusicMute())
            WildBuffaloSoundManager.Instance.StopMusic(soundName);
    }
    ...
    public void PlaySpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!WildBuffaloSoundManager.Instance.IsSoundMute())
            WildBuffaloSoundManager.Instance.SpinPlayMusic(soundName);
    }
    ...
    public void StopSpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!WildBuffaloSoundManager.Instance.IsSoundMute())
            WildBuffaloSoundManager.Instance.SpinStopMusic(soundName);
    }

    private void SoundActive(bool soundActive)
    {
        WildBuffaloSoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        WildBuffaloSoundManager.Instance.MuteMusic(!musicActive);
    ...
    private void OnClickStop()
    {
        ...
        StopCurrentSFX();
        //StopSpinMusic("Spin");
        WildBuffaloSlotMachine.Instance.isStopBtnPressed = true;
        WildBuffaloSlotMachine.Instance.StopWithResult();

        if (WildBuffaloAutoSpinController.isAutoSpinning)
        {
