...
[RequireComponent(typeof(GoldenDragonBetController))]
[RequireComponent(typeof(GoldenDragonRulesPopupController))]
[RequireComponent(typeof(GoldenDragonAutoSpinController))]
public class GoldenDragonUIManager : GameBetServices
{
    #region Variables
    public static GoldenDragonUIManager Instance;
    ...
    [Header("Spin Buttons")]
    [SerializeField] public GoldenDragonUIButtonController spinButton;
    [SerializeField] public GoldenDragonUIButtonController stopButton;
    ...
    private GoldenDragonBetController betController;
    private GoldenDragonRulesPopupController rulesPopupController;
    private GoldenDragonAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<GoldenDragonBetController>();
        rulesPopupController = GetComponent<GoldenDragonRulesPopupController>();
        autoSpinController = GetComponent<GoldenDragonAutoSpinController>();
    ...
    public void PlaySound(string soundName)
    {
        ...
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.PlaySFX(soundName);
    }
    public void StopCurrentSFX()
    {
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.StopSFX();
    }
    public void PlayMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsMusicMute())
            GoldenDragonSoundManager.Instance.PlayMusic(soundName);
    }
    ...
    public void StopMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsMusicMute())
            GoldenDragonSoundManager.Instance.StopMusic(soundName);
    }
    public void PlaySpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.PlaySpinMusic(soundName);
    }
    public void StopSpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.StopSpinMusic(soundName);
    }
    private void PlayWinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.PlayWinMusic(soundName);
    }
    ...
    private void StopWinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!GoldenDragonSoundManager.Instance.IsSoundMute())
            GoldenDragonSoundManager.Instance.StopWinMusic(soundName);
    }
    private void SoundActive(bool soundActive)
    {
        GoldenDragonSoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        GoldenDragonSoundManager.Instance.MuteMusic(!musicActive);
    ...
    {
        if (GoldenDragonSlotMachine.Instance.InSpin) return;
    ...
    private void OnClickStop()
    {
        PlaySound("Button");
        StopSpinMusic("Spin");
        if (GoldenDragonSlotMachine.Instance.isFreeGame)
        {
        ...
        GoldenDragonSlotMachine.Instance.isStopBtnPressed = true;

        if (GoldenDragonAutoSpinController.isAutoSpinning)
        {
            autoSpinController.CancelAutoSpin();
        }
        GoldenDragonSlotMachine.Instance.StopWithResult();
        GoldenDragonSlotMachine.Instance.InvokeStop();
    }
    ...
    public void TextAnimation(float target, float duration, TMP_Text textToAnimate)
    {
        ...
        freeGameWinAmount += GoldenDragonSlotMachine.Instance.winAmount;
