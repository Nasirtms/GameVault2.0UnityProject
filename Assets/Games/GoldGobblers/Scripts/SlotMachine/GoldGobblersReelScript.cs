...
public class GoldGobblersReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private GoldGobblersGameSettings settings;

    [Header("References")]
    public List<GoldGobblersSlotScript> slots;
    ...
    // Direction control
    private GoldGobblersSpinDirection currentSpinDirection = GoldGobblersSpinDirection.Down;
    ...
    {
        settings = GoldGobblersSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 5)
        {
            slots.Clear();
            slots = new List<GoldGobblersSlotScript>(GetComponentsInChildren<GoldGobblersSlotScript>());
        }

        foreach (GoldGobblersSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        int rowLimit = 0;
        if (reelIndex >= GoldGobblersSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = GoldGobblersSlotMachine.Instance.spinSymbolMatrix[reelIndex];

        finalResultSymbols = symbols;
        allowSymbolChanges = false;

        if (GoldGobblersSlotMachine.Instance.isFreeGame && GoldGobblersSlotMachine.Instance.hasRedFreeGameStarted)
        {
            rowLimit = 5;
        }
        else rowLimit = 3;
        ...
        for (int rowIndex = 0; rowIndex < rowLimit; rowIndex++)
        {
            ...
            var res = GoldGobblersSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                GoldGobblersSlotMachine.Instance.isResultReceived = true;

                var slot = slots[rowIndex + 2]; // Make sure slots[1], [2], [3] are the visible ones
    ...
    public void SetSpinDirection(GoldGobblersSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == GoldGobblersSpinDirection.Up)
        {
    ...
    public void ForceClampToTop()
    {
        ...
        if (GoldGobblersSlotMachine.Instance.isFreeGame && GoldGobblersSlotMachine.Instance.hasRedFreeGameStarted)
        {
            topPosition.y = settings.slotSettings.TopYPositionRedFreeSpin;
        }
        else
        {
            topPosition.y = settings.slotSettings.TopYPosition;
