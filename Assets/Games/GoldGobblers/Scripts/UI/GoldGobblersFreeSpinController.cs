...
public class GoldGobblersFreeSpinController : MonoBehaviour
{
    #region Variables
    ...
    private IEnumerator FreeSpinLoop()
    {
        ...
        while (freeSpinDone < totalFreeSpins)
        {
            ...
            {
                yield return new WaitUntil(() => !GoldGobblersSlotMachine.Instance.hasNewFreeGameTriggeredInBetween);
                yield return new WaitForSeconds(1f);
            }
            yield return new WaitUntil(() => !GoldGobblersSlotMachine.Instance.showingTunnelSlotAnimation);
    ...
    private void EndFreeSpins()
    {
        ...
        GoldGobblersSlotMachine.Instance.isFreeGame = false;

        GoldGobblersFreeGameTransitionController.Instance.EndFreeSpinTransition();
    }
