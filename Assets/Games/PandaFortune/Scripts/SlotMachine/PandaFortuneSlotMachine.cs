...
public class PandaFortuneSlotMachine : BaseSlotMachine
{
    ...
    private IEnumerator StartSpin()
    {
        ...
        PandaFortuneUIManager.Instance.PlaySpinMusic("Spin");
        ...
        PandaFortunePaylineController.Instance.StopPaylineDisplay();
    ...
    private IEnumerator WaitUntilResultAndThenStop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            PandaFortuneUIManager.Instance.StopSpinMusic("Spin");
            isSpinAgain = true;
    ...
    private void ProcessSpinResult()
    {
        ...
        if (winAmount > 0)
        {
            PandaFortuneUIManager.Instance.PlaySound("Win");
        }
        // Spin complete
        isSpinAgain = true;
        InSpin = false;
    ...
    private void SubmitFrozenColumns()
    {
        frozenColumns.Clear();
        ...
        // Free Game 1: the "current spin frozen" reel
        if (frozenIndexThisSpin >= 0 && frozenIndexThisSpin < reels.Count)
            frozenColumns.Add(frozenIndexThisSpin);

        // Free Game 2: potentially multiple frozen reels
        if (isFreeGameTwo && frozenReelsFreeGame2 != null)
        {
            int len = Mathf.Min(frozenReelsFreeGame2.Length, reels.Count);
            for (int i = 0; i < len; i++)
            {
                if (frozenReelsFreeGame2[i] && !frozenColumns.Contains(i))
                    frozenColumns.Add(i);
            }
        }
