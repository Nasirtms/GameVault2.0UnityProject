...
public class PandaFortuneReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private PandaFortuneGameSettings settings;

    [Header("References")]
    public List<PandaFortuneSlotScript> slots;
    ...
    // Direction control
    private PandaFortuneSpinDirection currentSpinDirection = PandaFortuneSpinDirection.Down;
    ...
    {
        settings = PandaFortuneSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<PandaFortuneSlotScript>(GetComponentsInChildren<PandaFortuneSlotScript>());
        }

        foreach (PandaFortuneSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= PandaFortuneSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = PandaFortuneSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = PandaFortuneSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                PandaFortuneSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(PandaFortuneSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == PandaFortuneSpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= PandaFortuneSlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = PandaFortuneSlotMachine.GetResourceById("wild");
    ...
    private void DoNotSetWildOnSlotsOutOfMask()
    {
        ...
        for (int i = 0; i < slots.Count; i++)
        {
            ...
            if (slot.slotType == PandaFortuneSlotType.Wild && (i == 0 || i == 4))
            {
