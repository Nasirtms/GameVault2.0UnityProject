...
using UnityEngine;
public class LuckySevenReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private LuckySevenGameSettings settings;

    [Header("References")]
    public List<LuckySevenSlotScript> slots;
    ...
    // Direction control
    private LuckySevenSpinDirection currentSpinDirection = LuckySevenSpinDirection.Down;
    ...
    {
        settings = LuckySevenSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<LuckySevenSlotScript>(GetComponentsInChildren<LuckySevenSlotScript>());
        }
    ...
    {
        foreach (LuckySevenSlotScript slot in slots)
        {
    ...
    {
        if (reelIndex >= LuckySevenSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = LuckySevenSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = LuckySevenSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                LuckySevenSlotMachine.Instance.isResultReceived = true;
                var slot = slots[rowIndex + 2]; // Make sure slots[2], [3], [4] are the visible ones

                slot.SetType(res.Value);
            }
        }

        var extraSlot = LuckySevenSlotMachine.GetResourceById("Empty");
        if (extraSlot.HasValue)
        {
            bool centerIsEmpty = slots[3].slotType == LuckySevenSlotType.Empty;

            if (!centerIsEmpty)
            {
                slots[0].SetType(
                LuckySevenSlotMachine.CachedRealSymbols[
                    Random.Range(0, LuckySevenSlotMachine.CachedRealSymbols.Count)]);
    ...
    public void SetSpinDirection(LuckySevenSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == LuckySevenSpinDirection.Up)
        {
    ...
    private LuckySevenSlotResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = LuckySevenSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return LuckySevenSlotMachine.CachedEmptySymbol.Value;
        }
    }
