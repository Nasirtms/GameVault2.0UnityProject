...
public class WheelOfFortuneReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<WheelOfFortuneSlotScript> slots;
    private RectTransform _rectTransform;
    private WheelOfFortuneSpinSettings _spinSettings;
    private Vector2 _targetPos;
    private WheelOfFortuneSpinDirection _spinDirection;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<WheelOfFortuneSlotScript>(GetComponentsInChildren<WheelOfFortuneSlotScript>());
        ...
        _spinSettings = WheelOfFortuneSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        WheelOfFortuneSlotMachine.Instance.isResultReceived = false;
    }
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = WheelOfFortuneSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = WheelOfFortuneSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
            ...
            if (res.HasValue)
            {
                if (_clampedDown)
                {
                    if (_spinDirection == WheelOfFortuneSpinDirection.Downwards)
                    {
                        row -= 1;
                    }
                    else
        ...
        var extraSlot = WheelOfFortuneSlotMachine.GetResourceById("Empty");
    ...
    public WheelOfFortuneSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == WheelOfFortuneSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        if (_spinSettings.spinDirection == WheelOfFortuneSpinDirection.Random)
        {
            _spinDirection = Random.Range(0, 2) == 0 ? WheelOfFortuneSpinDirection.Downwards : WheelOfFortuneSpinDirection.Upwards;
        }
        else
        {
        ...
        _acceleration = acceleration <= 0 ? WheelOfFortuneGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? WheelOfFortuneGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private WheelOfFortuneSlotResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = WheelOfFortuneSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return WheelOfFortuneSlotMachine.CachedEmptySymbol.Value;
        }
    }
