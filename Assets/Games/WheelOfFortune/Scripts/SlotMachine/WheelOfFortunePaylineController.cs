...
public class WheelOfFortunePaylineController : MonoBehaviour
{
    ...
    public void StartPaylineLoop(bool isFreeSpin)
    {
        StopPaylineLoop();
        activePaylines.Clear();
        resultFreeSpin = isFreeSpin;
        ...
        if (activePaylines.Count == 0 && !resultFreeSpin)
        {
    ...
    private IEnumerator AnimateCycle()
    {
        ...
        while (isShowing)
        {
            ...
            if (resultFreeSpin)
            {
                //WheelOfFortuneUIManager.Instance.PlaySound("FreeSpinStart");

                WheelOfFortuneSlotMachine.Instance.firstFreeSpin = true;
