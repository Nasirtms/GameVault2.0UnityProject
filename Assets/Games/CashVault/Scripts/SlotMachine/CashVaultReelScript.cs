...
public class CashVaultReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private CashVaultGameSettings settings;

    [Header("References")]
    public List<CashVaultSlotScript> slots;
    ...
    // Direction control
    private CashVaultSpinDirection currentSpinDirection = CashVaultSpinDirection.Down;
    ...
    {
        settings = CashVaultSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<CashVaultSlotScript>(GetComponentsInChildren<CashVaultSlotScript>());
        }

        foreach (CashVaultSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= CashVaultSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = CashVaultSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = CashVaultSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                CashVaultSlotMachine.Instance.isResultReceived = true;
                var slot = slots[rowIndex + 1]; // Make sure slots[1], [2], [3] are the visible ones

                if (res.Value.slotType == CashVaultSlotType.Sphere)
                {
    ...
    public void SetSpinDirection(CashVaultSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == CashVaultSpinDirection.Up)
        {
