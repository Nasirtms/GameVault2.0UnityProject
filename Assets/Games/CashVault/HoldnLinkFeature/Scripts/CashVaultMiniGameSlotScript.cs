...
public class CashVaultMiniGameSlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public CashVaultMiniGameSlotType slotType;
    [HideInInspector] public CashVaultMiniGameSlotResource currentResource;
    ...
    {
        var random = CashVaultMiniGameSlotMachine.Instance.settings.slotResources[UnityEngine.Random.Range(0, CashVaultMiniGameSlotMachine.Instance.settings.slotResources.Count)];
        SetType(random, false);
    }
    public void SetType(CashVaultMiniGameSlotResource newType, bool finalResult)
    {
    ...
    {
        var targetbox = CashVaultMiniGameManager.Instance.Target_Text;
        ...
        CashVaultMiniGameManager.Instance.Target_Text.text = $"${WinCollectedAmount.ToString("F2")}";
