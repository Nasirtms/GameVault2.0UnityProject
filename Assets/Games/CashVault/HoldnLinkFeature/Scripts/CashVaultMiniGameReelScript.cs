...
public class CashVaultMiniGameReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private CashVaultMiniGameSettings settings;

    [Header("References")]
    public List<CashVaultMiniGameSlotScript> slots;
    ...
    private CashVaultMiniGameSpinDirection currentSpinDirection = CashVaultMiniGameSpinDirection.Down;
    ...
    public void ApplyLockVisual(bool locked)
    {
        ...
        var emptyRes = CashVaultMiniGameSlotMachine.GetResourceById("empty");
    ...
    {
        settings = CashVaultMiniGameSlotMachine.Instance.settings;

        if (slots == null) slots = new List<CashVaultMiniGameSlotScript>(4);
        if (slots.Count != 4)
        {
            slots.Clear();
            slots.AddRange(GetComponentsInChildren<CashVaultMiniGameSlotScript>(true));
        }
    ...
    public void SetSpinDirection(CashVaultMiniGameSpinDirection direction) => currentSpinDirection = direction;
    ...
    private IEnumerator SmoothWindUp()
    {
        ...
        float sign = (currentSpinDirection == CashVaultMiniGameSpinDirection.Up) ? -1f : 1f;
    ...
    {
        float dir = (currentSpinDirection == CashVaultMiniGameSpinDirection.Up) ? 1f : -1f;
    ...
    {
        if (reelIndex >= CashVaultMiniGameSlotMachine.Instance.spinSymbolMatrix.Count)
            return;

        finalResultSymbols = CashVaultMiniGameSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            var res = CashVaultMiniGameSlotMachine.GetResourceById(symbolData.id);
            if (!res.HasValue) continue;

            CashVaultMiniGameSlotMachine.Instance.isResultReceived = true;

            var slot = slots[row + 1];

            if (res.Value.slotType == CashVaultMiniGameSlotType.Sphere)
