...
public class CashVaultMiniGameManager : MonoBehaviour
{
    #region Variables
    public static CashVaultMiniGameManager Instance;
    ...
    {
        CashVaultSlotMachine.Instance.isMiniGame = true;
    ...
    private IEnumerator StartReSpinGame()
    {
        ...
        CashVaultMiniGameSlotMachine.Instance.lockedReelIndexes.Clear();
        if (CashVaultSlotMachine.Instance.miniGame1)
        {
            var mini = CashVaultMiniGameSlotMachine.Instance;

            mini.lockedReelIndexes.AddRange(CashVaultSlotMachine.Instance.miniGameLockedReels);
        ...
        Debug.Log("LockedReelIndexes : " + CashVaultMiniGameSlotMachine.Instance.lockedReelIndexes);
        CashVaultSlotMachine.Instance.BaseGameReelsObject.SetActive(false);
        CashVaultSlotMachine.Instance.MiniGameReelsObject.SetActive(true);
    ...
    private IEnumerator EndReSpinGame()
    {
        ...
        yield return new WaitForSeconds(1f);
        CashVaultSlotMachine.Instance.MiniGameReelsObject.SetActive(false);
        CashVaultSlotMachine.Instance.BaseGameReelsObject.SetActive(true);
    ...
    {
        yield return new WaitForSeconds(1f);
        currentLockedCount = CashVaultMiniGameSlotMachine.Instance.lockedReelIndexes.Count;
        while (featureRunning && reSpinsLeft > 0)
        {
            if (firstReSpin)
            {
                CashVaultMiniGameSlotMachine.Instance.firstReSpin = false;
                firstReSpin = false;
            }
            else
            {
            ...
            if (CashVaultMiniGameSlotMachine.Instance.AreAllReelsLocked())
                break;

            int previousLockedCount = CashVaultMiniGameSlotMachine.Instance.lockedReelIndexes.Count;
            ...
            yield return new WaitUntil(() => CashVaultMiniGameSlotMachine.Instance.isSpinAgain);
    ...
    private void EndReSpins()
    {
        ...
        reSpinsLeft = 0;
        isMiniGame = false;
        CashVaultSlotMachine.Instance.isMiniGame = false;
    ...
    private IEnumerator CollectSpheresToTarget()
    {
        ...
        foreach (int reelIndex in reelsLocked)
        {
            ...
            if (slot.slotType != CashVaultMiniGameSlotType.Sphere)
                continue;
    ...
    private void ResetMiniGameState()
    {
        ...
        foreach (var reel in CashVaultMiniGameSlotMachine.Instance.reels)
        {
