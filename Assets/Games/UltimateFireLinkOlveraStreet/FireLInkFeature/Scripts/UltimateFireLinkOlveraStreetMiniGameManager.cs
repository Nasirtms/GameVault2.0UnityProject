...
public class UltimateFireLinkOlveraStreetMiniGameManager : MonoBehaviour
{
    #region Variables
    public static UltimateFireLinkOlveraStreetMiniGameManager Instance;
    ...
    {
        UltimateFireLinkOlveraStreetSlotMachine.Instance.isMiniGame = true;
    ...
    private IEnumerator StartReSpinGame()
    {
        ...
        UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.lockedReelIndexes.Clear();
        if (UltimateFireLinkOlveraStreetSlotMachine.Instance.miniGame1)
        {
            var mini = UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance;

            mini.lockedReelIndexes.AddRange(UltimateFireLinkOlveraStreetSlotMachine.Instance.miniGameLockedReels);
        ...
        Debug.Log("LockedReelIndexes : " + UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.lockedReelIndexes);
        UltimateFireLinkOlveraStreetSlotMachine.Instance.BaseGameReelsObject.SetActive(false);
        UltimateFireLinkOlveraStreetSlotMachine.Instance.MiniGameReelsObject.SetActive(true);
        ...
        UltimateFireLinkOlveraStreetUIManager.Instance.UpdateButtons("Free Spin");
    ...
    private IEnumerator EndReSpinGame()
    {
        ...
        UltimateFireLinkOlveraStreetUIManager.Instance.UpdateButtons("Transition End");

        PopupAnimation(miniGameWinFrame, 1f, 1f, true);

        yield return new WaitForSeconds(1f);
        UltimateFireLinkOlveraStreetSlotMachine.Instance.MiniGameReelsObject.SetActive(false);
        UltimateFireLinkOlveraStreetSlotMachine.Instance.BaseGameReelsObject.SetActive(true);
        ...
        //else
        //{
        UltimateFireLinkOlveraStreetUIManager.Instance.UpdateButtons("Free Spin End");
        //}
        ResetMiniGameState();
    }
    ...
    {
        yield return new WaitForSeconds(1f);
        currentLockedCount = UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.lockedReelIndexes.Count;
        while (featureRunning && reSpinsLeft > 0)
        {
            if (firstReSpin)
            {
                UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.firstReSpin = false;
                firstReSpin = false;
            }
            else
            {
            ...
            if (UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.AreAllReelsLocked())
                break;

            int previousLockedCount = UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.lockedReelIndexes.Count;
            ...
            yield return new WaitUntil(() => UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.isSpinAgain);
    ...
    private IEnumerator CollectSpheresToTarget()
    {
        ...
        foreach (int reelIndex in reelsLocked)
        {
            ...
            if (slot.slotType != UltimateFireLinkOlveraStreetMiniGameSlotType.Mini)
                continue;
    ...
    private void ResetMiniGameState()
    {
        ...
        foreach (var reel in UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.reels)
        {
