...
public class UltimateFireLinkOlveraStreetMiniGameReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkOlveraStreetMiniGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkOlveraStreetMiniGameSlotScript> slots;
    ...
    private UltimateFireLinkOlveraStreetMiniGameSpinDirection currentSpinDirection = UltimateFireLinkOlveraStreetMiniGameSpinDirection.Down;
    ...
    {
        var sphereRes = UltimateFireLinkOlveraStreetMiniGameSlotMachine.GetResourceById("sphere");
        var emptyRes = UltimateFireLinkOlveraStreetMiniGameSlotMachine.GetResourceById("empty");
    ...
    {
        settings = UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.settings;

        if (slots == null) slots = new List<UltimateFireLinkOlveraStreetMiniGameSlotScript>(4);
        if (slots.Count != 4)
        {
            slots.Clear();
            slots.AddRange(GetComponentsInChildren<UltimateFireLinkOlveraStreetMiniGameSlotScript>(true));
        }
    ...
    public void SetSpinDirection(UltimateFireLinkOlveraStreetMiniGameSpinDirection direction) => currentSpinDirection = direction;
    ...
    private IEnumerator SmoothWindUp()
    {
        ...
        float sign = (currentSpinDirection == UltimateFireLinkOlveraStreetMiniGameSpinDirection.Up) ? -1f : 1f;
    ...
    {
        float dir = (currentSpinDirection == UltimateFireLinkOlveraStreetMiniGameSpinDirection.Up) ? 1f : -1f;
    ...
    {
        if (reelIndex >= UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.spinSymbolMatrix.Count)
            return;

        finalResultSymbols = UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            var res = UltimateFireLinkOlveraStreetMiniGameSlotMachine.GetResourceById(symbolData.id);
            if (!res.HasValue) continue;

            UltimateFireLinkOlveraStreetMiniGameSlotMachine.Instance.isResultReceived = true;

            var slot = slots[row + 1];

            if (res.Value.slotType == UltimateFireLinkOlveraStreetMiniGameSlotType.Mini)
