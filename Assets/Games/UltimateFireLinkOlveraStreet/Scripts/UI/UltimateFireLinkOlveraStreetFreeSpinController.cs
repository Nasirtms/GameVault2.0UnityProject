...
public class UltimateFireLinkOlveraStreetFreeSpinController : MonoBehaviour
{
    #region Variables
    ...
    public void StartFreeSpins()
    {
        ...
        UltimateFireLinkOlveraStreetSlotMachine.Instance.isFreeGame = true;
        isFreeGame = true;
        firstSpin = true;
        freeSpinDone = 0;
    ...
    private IEnumerator FreeSpinLoop()
    {
        ...
        while (freeSpinDone < totalFreeSpins)
        {
            ...
            Debug.Log("Deepak has a spin again - 10" + UltimateFireLinkOlveraStreetSlotMachine.Instance.isSpinAgain);

            yield return new WaitUntil(() => UltimateFireLinkOlveraStreetSlotMachine.Instance.isSpinAgain);
            //yield return new WaitForSeconds(4f);
            if (freeSpinDone == 0)
            {
                UltimateFireLinkOlveraStreetSlotMachine.Instance.firstFreeSpin = false;
            }
    ...
    private void EndFreeSpins()
    {
        ...
        ResetFreeSpins();
        isFreeGame = false;
        UltimateFireLinkOlveraStreetFreeGameTransitionController.Instance.EndFreeSpin();
        UltimateFireLinkOlveraStreetSlotMachine.Instance.isFreeGame = false;
        UltimateFireLinkOlveraStreetUIManager.Instance.UpdateButtons("FreeSpinEnd");
