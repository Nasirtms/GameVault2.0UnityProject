...
public class UltimateFireLinkOlveraStreetSlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public UltimateFireLinkOlveraStreetSlotType slotType;
    [HideInInspector] public UltimateFireLinkOlveraStreetSlotResource currentResource;
    ...
    {
        var allResources = UltimateFireLinkOlveraStreetSlotMachine.Instance.settings.slotResources;

        if (allResources == null || allResources.Count == 0)
            return;

        List<UltimateFireLinkOlveraStreetSlotResource> filtered = new List<UltimateFireLinkOlveraStreetSlotResource>();

        foreach (var r in allResources)
        {
            if (r.slotType != UltimateFireLinkOlveraStreetSlotType.Freegames &&
                r.slotType != UltimateFireLinkOlveraStreetSlotType.Minor &&
                r.slotType != UltimateFireLinkOlveraStreetSlotType.Major &&
                r.slotType != UltimateFireLinkOlveraStreetSlotType.Mega &&
                r.slotType != UltimateFireLinkOlveraStreetSlotType.Mini)
            {
                filtered.Add(r);
            }
        }
    ...
    public void SetType(UltimateFireLinkOlveraStreetSlotResource newType)
    {
