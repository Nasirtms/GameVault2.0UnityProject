...
public class UltimateFireLinkOlveraStreetReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkOlveraStreetGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkOlveraStreetSlotScript> slots;
    ...
    // Direction control
    private UltimateFireLinkOlveraStreetSpinDirection currentSpinDirection = UltimateFireLinkOlveraStreetSpinDirection.Down;
    ...
    {
        settings = UltimateFireLinkOlveraStreetSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<UltimateFireLinkOlveraStreetSlotScript>(GetComponentsInChildren<UltimateFireLinkOlveraStreetSlotScript>());
        }

        foreach (UltimateFireLinkOlveraStreetSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= UltimateFireLinkOlveraStreetSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = UltimateFireLinkOlveraStreetSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = UltimateFireLinkOlveraStreetSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                UltimateFireLinkOlveraStreetSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(UltimateFireLinkOlveraStreetSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == UltimateFireLinkOlveraStreetSpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= UltimateFireLinkOlveraStreetSlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = UltimateFireLinkOlveraStreetSlotMachine.GetResourceById("wild");
