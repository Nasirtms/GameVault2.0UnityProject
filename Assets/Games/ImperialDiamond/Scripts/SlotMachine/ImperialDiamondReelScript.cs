...
public class ImperialDiamondReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<ImperialDiamondSlotScript> slots;
    private RectTransform _rectTransform;
    private ImperialDiamondSpinSettings _spinSettings;
    ...
    // Events
    public delegate void ImperialDiamondReelEvents(int index);
    public static event ImperialDiamondReelEvents OnSpinStart;
    public static event ImperialDiamondReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<ImperialDiamondSlotScript>(GetComponentsInChildren<ImperialDiamondSlotScript>());
        }
        ...
        _spinSettings = ImperialDiamondSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        ImperialDiamondSlotMachine.Instance.isResultReceived = false;
    }
    ...
    {
        if (reelIndex >= ImperialDiamondSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            //Debug.LogError($"❌ No spin data for reel {reelIndex}!");
            return;
        }

        var symbols = ImperialDiamondSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = ImperialDiamondSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
        ...
        var extraSlot = ImperialDiamondSlotMachine.GetResourceById("Empty");
        if (_clampedDown)
        {
            if (slots[1].type == ImperialDiamondSlotType.Empty)
            {
                slots[0].SetType(ImperialDiamondSlotMachine.CachedRealSymbols[Random.Range(0, ImperialDiamondSlotMachine.CachedRealSymbols.Count)]);
                slots[4].SetType(ImperialDiamondSlotMachine.CachedRealSymbols[Random.Range(0, ImperialDiamondSlotMachine.CachedRealSymbols.Count)]);
            }
            else
            {
        ...
        {
            if (slots[2].type == ImperialDiamondSlotType.Empty)
            {
                slots[1].SetType(ImperialDiamondSlotMachine.CachedRealSymbols[Random.Range(0, ImperialDiamondSlotMachine.CachedRealSymbols.Count)]);
                slots[5].SetType(ImperialDiamondSlotMachine.CachedRealSymbols[Random.Range(0, ImperialDiamondSlotMachine.CachedRealSymbols.Count)]);
            }
            else
            {
    ...
    public ImperialDiamondSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == ImperialDiamondSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? ImperialDiamondGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? ImperialDiamondGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private void OnClampComplete()
    {
        ...
        //ImperialDiamondUIManager.Instance.PlaySound("ReelStop");
        OnSpinComplete?.Invoke(this._index);
    }

    private ImperialDiamondResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = ImperialDiamondSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return ImperialDiamondSlotMachine.CachedEmptySymbol.Value;
        }
    }
