...
public class SecretsOfChristmasReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private SecretsOfChristmasGameSettings settings;

    [Header("References")]
    public List<SecretsOfChristmasSlotScript> slots;
    ...
    // Direction control
    private SecretsOfChristmasSpinDirection currentSpinDirection = SecretsOfChristmasSpinDirection.Down;
    ...
    {
        settings = SecretsOfChristmasSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 7)
        {
            slots.Clear();
            slots = new List<SecretsOfChristmasSlotScript>(GetComponentsInChildren<SecretsOfChristmasSlotScript>());
        }

        foreach (SecretsOfChristmasSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= SecretsOfChristmasSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = SecretsOfChristmasSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = SecretsOfChristmasSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                SecretsOfChristmasSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(SecretsOfChristmasSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == SecretsOfChristmasSpinDirection.Up)
        {
