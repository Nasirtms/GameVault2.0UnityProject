...
public class SecretsOfChristmasSlotMachine : BaseSlotMachine
{
    #region Variables
    public static SecretsOfChristmasSlotMachine Instance;

    [Header("Machine References")]
    public SecretsOfChristmasGameSettings settings;
    public List<SecretsOfChristmasReelScript> reels;
    [SerializeField] private SecretsOfChristmasBetController betController;
    ...
    [Header("Result")]
    [ShowInInspector][Sirenix.OdinInspector.ReadOnly] public SecretsOfChristmasSlotType[,] resultMatrix;
    #endregion
    ...
    private void OnSpinResultReceived(BaseSpinResult result)
    {
        ...
        SecretsOfChristmasUIManager.Instance.SetStopInteractable(true);
    }
    #endregion
    ...
    private IEnumerator StartSpin()
    {
        if (!isFreeGame || firstFreeSpin)
        {
            ...
            SecretsOfChristmasUIManager.Instance.UpdateWinAmount(0f);
            freeSpinWinAmount = 0;
            winAmount = 0f;
        }
        ...
        SecretsOfChristmasUIManager.Instance.StopCurrentSFX();

        ClearPaylines();
        //SecretsOfChristmasPaylineController.Instance.StopPaylines();
        //SecretsOfChristmasPaylineController.Instance.ClearPaylineData();
        SecretsOfChristmasUIManager.Instance.SetStopInteractable(false);

        if (settings.spinSettings.startSpin == SecretsOfChristmasSpinMode.SpinAll)
        {
            foreach (SecretsOfChristmasReelScript reel in reels)
            {
                if (reel != null)
                {
    ...
    private IEnumerator WaitUntilResultAndThenStop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            if (isFreeGame)
            {
                //SecretsOfChristmasFreeGameTransitionController.Instance.NetworkErrorFreeSpin();
            }
            else if (SecretsOfChristmasAutoSpinController.isAutoSpinning)
            {
                SecretsOfChristmasUIManager.Instance.CancelAutoSpin();
            }
            else
            {
    ...
    public void Stop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            if (!SecretsOfChristmasAutoSpinController.isAutoSpinning && !isFreeGame)
            {
                SecretsOfChristmasUIManager.Instance.UpdateButtons("Stop");
            }
    ...
    private IEnumerator StopReelsWithResultRoutine()
    {
        ...
        if (settings.spinSettings.endSpin == SecretsOfChristmasSpinMode.SpinAll)
        {
    ...
    private void ProcessSpinResult()
    {
        ...
        if (isFreeGame && winAmount > 0)
        {
            ...
            SecretsOfChristmasUIManager.Instance.UpdateWinAmount(winAmount, true);
        }
        else if (winAmount > 0)
        {
        ...
        if (hasPaylines)
        {
            ...
            {
                SecretsOfChristmasPaylineResult result = new SecretsOfChristmasPaylineResult(payline.paylineIndex, payline.count, payline.winAmount);
                SecretsOfChristmasPaylineController.Instance.AddPaylineData(result);
            }
        }
        ...
        else if (!SecretsOfChristmasAutoSpinController.isAutoSpinning && !isFreeGame)
        {
            SecretsOfChristmasUIManager.Instance.UpdateButtons("Stop");
        }
        else if (isFreeGame)
    ...
    {
        SecretsOfChristmasPaylineController.Instance.StartPayline(isFreeGameReady);
    }
    #endregion
    ...
    private void SetReelDirection(SecretsOfChristmasReelScript reel)
    {
        SecretsOfChristmasSpinDirection direction = settings.spinSettings.spinDirection;

        if (direction == SecretsOfChristmasSpinDirection.Random)
        {
            direction = Random.value > 0.5f ? SecretsOfChristmasSpinDirection.Up : SecretsOfChristmasSpinDirection.Down;
        }
    ...
    public static SecretsOfChristmasSlotResource? GetResourceById(string id)
    {
