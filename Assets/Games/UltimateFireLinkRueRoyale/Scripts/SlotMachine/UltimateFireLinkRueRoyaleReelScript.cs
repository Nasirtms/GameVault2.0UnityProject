...
public class UltimateFireLinkRueRoyaleReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkRueRoyaleGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkRueRoyaleSlotScript> slots;
    ...
    // Direction control
    private UltimateFireLinkRueRoyaleSpinDirection currentSpinDirection = UltimateFireLinkRueRoyaleSpinDirection.Down;
    ...
    {
        settings = UltimateFireLinkRueRoyaleSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<UltimateFireLinkRueRoyaleSlotScript>(GetComponentsInChildren<UltimateFireLinkRueRoyaleSlotScript>());
        }

        foreach (UltimateFireLinkRueRoyaleSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= UltimateFireLinkRueRoyaleSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = UltimateFireLinkRueRoyaleSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = UltimateFireLinkRueRoyaleSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                UltimateFireLinkRueRoyaleSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(UltimateFireLinkRueRoyaleSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == UltimateFireLinkRueRoyaleSpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= UltimateFireLinkRueRoyaleSlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = UltimateFireLinkRueRoyaleSlotMachine.GetResourceById("wild");
