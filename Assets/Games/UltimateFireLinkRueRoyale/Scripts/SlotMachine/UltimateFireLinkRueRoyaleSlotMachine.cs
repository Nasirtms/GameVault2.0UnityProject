...
public class UltimateFireLinkRueRoyaleSlotMachine : BaseSlotMachine
{
    #region Variables

    public static UltimateFireLinkRueRoyaleSlotMachine Instance;

    [Header("Machine References")]
    public UltimateFireLinkRueRoyaleGameSettings settings;
    public List<UltimateFireLinkRueRoyaleReelScript> reels;
    [SerializeField] private UltimateFireLinkRueRoyaleBetController betController;
    ...
    private IEnumerator StartSpin()
    {
        ...
        if (!isFreeGame || firstFreeSpin)
        {
            ...
            UltimateFireLinkRueRoyaleUIManager.Instance.UpdateWinAmount(0f);
            freeSpinWinAmount = 0;
            winAmount = 0f;
        }

        UltimateFireLinkRueRoyaleSoundManager.Instance.PlayMusic("Spin");
        ...
        UltimateFireLinkRueRoyalePaylineController.Instance.ClearPaylineData();
        UltimateFireLinkRueRoyalePaylineController.Instance.StopPaylineDisplay();
        ...
        if (settings.spinSettings.startSpin == UltimateFireLinkRueRoyaleSpinMode.SpinAll)
        {
            for (int i = 0; i < reels.Count; i++)
            {
                UltimateFireLinkRueRoyaleReelScript reel = reels[i];
    ...
    private IEnumerator WaitUntilResultAndThenStop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            else if (UltimateFireLinkRueRoyaleAutoSpinController.isAutoSpinning)
            {
                UltimateFireLinkRueRoyaleUIManager.Instance.CancelAutoSpin();
            }
            else
            {
                UltimateFireLinkRueRoyaleUIManager.Instance.UpdateButtons("Idle");
            }
            UltimateFireLinkRueRoyaleSoundManager.Instance.StopMusic("Spin");
            isSpinAgain = true;
    ...
    public void Stop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            if (!UltimateFireLinkRueRoyaleAutoSpinController.isAutoSpinning && !isFreeGame)
            {
                UltimateFireLinkRueRoyaleUIManager.Instance.UpdateButtons("Idle");
            }
    ...
    private IEnumerator StopReelsWithResultRoutine()
    {
        ...
        if (settings.spinSettings.endSpin == UltimateFireLinkRueRoyaleSpinMode.SpinAll)
        {
            for (int i = 0; i < reels.Count; i++)
            {
    ...
    private void ProcessSpinResult()
    {
        ...
        if (isFreeGame && winAmount > 0)
        {
            ...
            UltimateFireLinkRueRoyaleUIManager.Instance.UpdateWinAmount(winAmount, true);
        }
        else if (winAmount > 0)
        {
        ...
        if ((currentSpinResult.paylineWins != null && currentSpinResult.paylineWins.Count > 0) || scatterCount >= 1)
        {
            if (currentSpinResult.paylineWins != null && currentSpinResult.paylineWins.Count > 0)
            {
                ...
                {
                    UltimateFireLinkRueRoyalePaylineResult result = new UltimateFireLinkRueRoyalePaylineResult(payline.paylineIndex, payline.count, payline.winAmount, payline.symbol);
                    UltimateFireLinkRueRoyalePaylineController.Instance.AddPaylineData(result);
        ...
        if (winAmount > 0)
        {
            UltimateFireLinkRueRoyaleUIManager.Instance.PlaySound("Win");
        }
        // Spin complete
        isSpinAgain = true;
        InSpin = false;
        ...
        if(isBonusGame)
        {
            UltimateFireLinkRueRoyaleUIManager.Instance.spinButton.GetButtonComponent().interactable = false;
            UltimateFireLinkRueRoyaleUIManager.Instance.autoButton.GetButtonComponent().interactable = false;
        }
    ...
    private void SetReelDirection(UltimateFireLinkRueRoyaleReelScript reel)
    {
        UltimateFireLinkRueRoyaleSpinDirection direction = settings.spinSettings.spinDirection;

        // If random direction, choose randomly for each reel
        if (direction == UltimateFireLinkRueRoyaleSpinDirection.Random)
        {
            direction = Random.value > 0.5f ? UltimateFireLinkRueRoyaleSpinDirection.Up : UltimateFireLinkRueRoyaleSpinDirection.Down;
        }
    ...
    {
        foreach (UltimateFireLinkRueRoyaleReelScript reel in reels)
        {
            if (reel != null)
            {
                reel.ForceClampToTop();
    ...
    {
        UltimateFireLinkRueRoyalePaylineController.Instance.StartPayline(scatterCount);
    }
    ...
    public static UltimateFireLinkRueRoyaleSlotResource? GetResourceById(string id)
    {
