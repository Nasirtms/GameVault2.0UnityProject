...
using UnityEngine;
public class SaharaRichesReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private SaharaRichesGameSettings settings;

    [Header("References")]
    public List<SaharaRichesSlotScript> slots;
    ...
    // Direction control
    private SaharaRichesSpinDirection currentSpinDirection = SaharaRichesSpinDirection.Down;
    ...
    public void Initialize()
    {
        ...
        {
            slots.Clear();
            slots = new List<SaharaRichesSlotScript>(GetComponentsInChildren<SaharaRichesSlotScript>());
        }
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = SaharaRichesSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = SaharaRichesSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                SaharaRichesSlotMachine.Instance.isResultReceived = true;
                var slot = slots[rowIndex + 1]; // Make sure slots[1], [2], [3] are the visible ones

                if (SaharaRichesSlotMachine.Instance.isDiamondSlot(res.Value.slotType))
                {
                    slot.UpdateDiamondAmount(SaharaRichesSlotMachine.Instance.currentSpinResult.jackpotWin.amount);
                }
                else
                {
                    slot.UpdateDiamondAmount(0f);
                }

                if (SaharaRichesSlotMachine.Instance.isCCSlot(res.Value.slotType))
                {
                    slot.UpdateCCAmount(symbolData.bonusPayout);
                }
                else
                {
    ...
    public void SetSpinDirection(SaharaRichesSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == SaharaRichesSpinDirection.Up)
        {
