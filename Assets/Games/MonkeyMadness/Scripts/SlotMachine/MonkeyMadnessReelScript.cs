...
public class MonkeyMadnessReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<MonkeyMadnessSlotScript> slots;
    private RectTransform _rectTransform;
    private MonkeyMadnessSpinSettings _spinSettings;
    ...
    // Events
    public delegate void MonkeyMadnessReelEvents(int index);
    public static event MonkeyMadnessReelEvents OnSpinStart;
    public static event MonkeyMadnessReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<MonkeyMadnessSlotScript>(GetComponentsInChildren<MonkeyMadnessSlotScript>());
    ...
    {
        MonkeyMadnessSlotMachine.Instance.isResultReceived = false;
    }
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = MonkeyMadnessSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = MonkeyMadnessSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
        ...
        var extraSlot = MonkeyMadnessSlotMachine.GetResourceById("Empty");
    ...
    public MonkeyMadnessSlotType GetSlotType(int index)
    {
    ...
    {
        slots[0].SetType(MonkeyMadnessSlotMachine.CachedRealSymbols[Random.Range(0, MonkeyMadnessSlotMachine.CachedRealSymbols.Count)]);
        ...
        if (_spinSettings.endSpin == MonkeyMadnessSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? MonkeyMadnessGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? MonkeyMadnessGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private MonkeyMadnessSlotResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = MonkeyMadnessSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return MonkeyMadnessSlotMachine.CachedEmptySymbol.Value;
        }
    }
    ...
    private void Update()
    {
        ...
        //if (_increaseSpeed || !MonkeyMadnessSlotMachine.Instance.isResultReceived)
        if (!canStopReel)
        {
