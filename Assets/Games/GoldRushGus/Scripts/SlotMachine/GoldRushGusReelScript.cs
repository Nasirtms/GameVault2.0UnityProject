...
public class GoldRushGusReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private GoldRushGusGameSettings settings;

    [Header("References")]
    public List<GoldRushGusSlotScript> slots;
    ...
    // Direction control
    private GoldRushGusSpinDirection currentSpinDirection = GoldRushGusSpinDirection.Down;
    ...
    {
        settings = GoldRushGusSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<GoldRushGusSlotScript>(GetComponentsInChildren<GoldRushGusSlotScript>());
        }

        foreach (GoldRushGusSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= GoldRushGusSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = GoldRushGusSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = GoldRushGusSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                GoldRushGusSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(GoldRushGusSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == GoldRushGusSpinDirection.Up)
        {
