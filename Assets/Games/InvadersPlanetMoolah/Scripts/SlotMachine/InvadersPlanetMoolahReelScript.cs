...
public class InvadersPlanetMoolahReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private InvadersPlanetMoolahGameSettings settings;

    [Header("References")]
    public List<InvadersPlanetMoolahSlotScript> slots;
    ...
    private InvadersPlanetMoolahSpinDirection currentSpinDirection = InvadersPlanetMoolahSpinDirection.Down;
    ...
    {
        settings = InvadersPlanetMoolahSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<InvadersPlanetMoolahSlotScript>(GetComponentsInChildren<InvadersPlanetMoolahSlotScript>());
        }

        foreach (InvadersPlanetMoolahSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= InvadersPlanetMoolahSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = InvadersPlanetMoolahSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = InvadersPlanetMoolahSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                InvadersPlanetMoolahSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(InvadersPlanetMoolahSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == InvadersPlanetMoolahSpinDirection.Up)
        {
