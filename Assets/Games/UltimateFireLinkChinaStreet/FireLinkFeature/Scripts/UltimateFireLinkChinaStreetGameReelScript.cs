...
public class UltimateFireLinkChinaStreetMiniGameReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkChinaStreetMiniGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkChinaStreetMiniGameSlotScript> slots;
    ...
    private UltimateFireLinkChinaStreetMiniGameSpinDirection currentSpinDirection = UltimateFireLinkChinaStreetMiniGameSpinDirection.Down;
    ...
    {
        var sphereRes = UltimateFireLinkChinaStreetMiniGameSlotMachine.GetResourceById("sphere");
        var emptyRes = UltimateFireLinkChinaStreetMiniGameSlotMachine.GetResourceById("empty");
    ...
    {
        settings = UltimateFireLinkChinaStreetMiniGameSlotMachine.Instance.settings;

        if (slots == null) slots = new List<UltimateFireLinkChinaStreetMiniGameSlotScript>(4);
        if (slots.Count != 4)
        {
            slots.Clear();
            slots.AddRange(GetComponentsInChildren<UltimateFireLinkChinaStreetMiniGameSlotScript>(true));
        }
    ...
    public void SetSpinDirection(UltimateFireLinkChinaStreetMiniGameSpinDirection direction) => currentSpinDirection = direction;
    ...
    private IEnumerator SmoothWindUp()
    {
        ...
        float sign = (currentSpinDirection == UltimateFireLinkChinaStreetMiniGameSpinDirection.Up) ? -1f : 1f;
    ...
    {
        float dir = (currentSpinDirection == UltimateFireLinkChinaStreetMiniGameSpinDirection.Up) ? 1f : -1f;
    ...
    {
        if (reelIndex >= UltimateFireLinkChinaStreetMiniGameSlotMachine.Instance.spinSymbolMatrix.Count)
            return;

        finalResultSymbols = UltimateFireLinkChinaStreetMiniGameSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            var res = UltimateFireLinkChinaStreetMiniGameSlotMachine.GetResourceById(symbolData.id);
            if (!res.HasValue) continue;

            UltimateFireLinkChinaStreetMiniGameSlotMachine.Instance.isResultReceived = true;

            var slot = slots[row + 1];

            if (res.Value.slotType == UltimateFireLinkChinaStreetMiniGameSlotType.FireLink100x)
