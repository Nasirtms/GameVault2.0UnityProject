...
public class UltimateFireLinkChinaStreetReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkChinaStreetGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkChinaStreetSlotScript> slots;
    ...
    // Direction control
    private UltimateFireLinkChinaStreetSpinDirection currentSpinDirection = UltimateFireLinkChinaStreetSpinDirection.Down;
    ...
    {
        settings = UltimateFireLinkChinaStreetSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<UltimateFireLinkChinaStreetSlotScript>(GetComponentsInChildren<UltimateFireLinkChinaStreetSlotScript>());
        }

        foreach (UltimateFireLinkChinaStreetSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= UltimateFireLinkChinaStreetSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = UltimateFireLinkChinaStreetSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = UltimateFireLinkChinaStreetSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                UltimateFireLinkChinaStreetSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(UltimateFireLinkChinaStreetSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == UltimateFireLinkChinaStreetSpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= UltimateFireLinkChinaStreetSlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = UltimateFireLinkChinaStreetSlotMachine.GetResourceById("wild");
