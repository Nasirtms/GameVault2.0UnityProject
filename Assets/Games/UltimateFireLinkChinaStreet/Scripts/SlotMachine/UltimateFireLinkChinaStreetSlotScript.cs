...
public class UltimateFireLinkChinaStreetSlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public UltimateFireLinkChinaStreetSlotType slotType;
    [HideInInspector] public UltimateFireLinkChinaStreetSlotResource currentResource;
    ...
    {
        var allResources = UltimateFireLinkChinaStreetSlotMachine.Instance.settings.slotResources;

        if (allResources == null || allResources.Count == 0)
            return;

        List<UltimateFireLinkChinaStreetSlotResource> filtered = new List<UltimateFireLinkChinaStreetSlotResource>();

        foreach (var r in allResources)
        {
            if (r.slotType != UltimateFireLinkChinaStreetSlotType.FreeGames &&
                r.slotType != UltimateFireLinkChinaStreetSlotType.FireLinkMinor &&
                r.slotType != UltimateFireLinkChinaStreetSlotType.FireLinkMajor &&
                r.slotType != UltimateFireLinkChinaStreetSlotType.FireLinkMega &&
                r.slotType != UltimateFireLinkChinaStreetSlotType.FireLinkMini)
            {
                filtered.Add(r);
            }
        }
    ...
    public void SetType(UltimateFireLinkChinaStreetSlotResource newType)
    {
