...
[RequireComponent(typeof(LifeOfLuxuryBetController))]
[RequireComponent(typeof(LifeOfLuxuryAutoSpinController))]
public class LifeOfLuxuryUIManager : GameBetServices
{
    #region Variables

    public static LifeOfLuxuryUIManager Instance;
    [SerializeField] private LifeOfLuxuryRulesPopupController rulesPopupController;
    ...
    private LifeOfLuxuryBetController betController;
    private LifeOfLuxuryAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<LifeOfLuxuryBetController>();
        autoSpinController = GetComponent<LifeOfLuxuryAutoSpinController>();
    ...
    public void PlaySound(string soundName)
    {
        ...
        if (!LifeOfLuxurySoundManager.Instance.IsSoundMute())
            LifeOfLuxurySoundManager.Instance.PlaySFX(soundName);
    }
    public void StopCurrentSFX()
    {
        if (!LifeOfLuxurySoundManager.Instance.IsSoundMute())
            LifeOfLuxurySoundManager.Instance.StopSFX();
    }
    public void PlayMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!LifeOfLuxurySoundManager.Instance.IsMusicMute())
            LifeOfLuxurySoundManager.Instance.PlayMusic(soundName);
    }
    ...
    public void StopMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!LifeOfLuxurySoundManager.Instance.IsMusicMute())
            LifeOfLuxurySoundManager.Instance.StopMusic(soundName);
    }
    public void PlaySpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!LifeOfLuxurySoundManager.Instance.IsSoundMute())
            LifeOfLuxurySoundManager.Instance.SpinPlayMusic(soundName);
    }
    ...
    public void StopSpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!LifeOfLuxurySoundManager.Instance.IsSoundMute())
            LifeOfLuxurySoundManager.Instance.SpinStopMusic(soundName);
    }

    private void SoundActive(bool soundActive)
    {
        LifeOfLuxurySoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        LifeOfLuxurySoundManager.Instance.MuteMusic(!musicActive);
    ...
    public void StartTitleLoop()
    {
        ...
        if (titleLoopCoroutine != null)
            StopCoroutine(titleLoopCoroutine);

        HideAll();

        index = 0;
        titleLoopCoroutine = StartCoroutine(TitleLoop());
    }
    ...
    public void StopTitleLoop()
    {
        ...
        {
            StopCoroutine(titleLoopCoroutine);
            titleLoopCoroutine = null;
        }

        HideAll();
    }

    private IEnumerator TitleLoop()
    {
        while (true)
        {
            ShowOnly(index);
    ...
    private void HideAll()
    {
        ...
        foreach (GameObject img in titleImages)
        {
            if (img != null)
                img.SetActive(false);
        }
