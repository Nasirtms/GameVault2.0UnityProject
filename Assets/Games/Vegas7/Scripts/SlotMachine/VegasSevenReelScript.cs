...
public class VegasSevenReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<VegasSevenSlotScript> slots;
    private RectTransform _rectTransform;
    private VegasSevenSpinSettings _spinSettings;
    ...
    // Events
    public delegate void VegasSevenReelEvents(int index);
    public static event VegasSevenReelEvents OnSpinStart;
    public static event VegasSevenReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<VegasSevenSlotScript>(GetComponentsInChildren<VegasSevenSlotScript>());
        ...
        _spinSettings = VegasSevenSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        VegasSevenSlotMachine.Instance.isResultReceived = false;
    }
    ...
    {
        if (reelIndex >= VegasSevenSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            //Debug.LogError($"❌ No spin data for reel {reelIndex}!");
            return;
        }

        var symbols = VegasSevenSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = VegasSevenSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;

            if (res.HasValue)
            {
                //VegasSevenSlotMachine.Instance.isResultReceived = true;
        ...
        var extraSlot = VegasSevenSlotMachine.GetResourceById("Empty");

        if (_clampedDown)
        {
            if (slots[1].type == VegasSevenSlotType.Empty)
            {
                slots[0].SetType(VegasSevenSlotMachine.CachedRealSymbols[Random.Range(0, VegasSevenSlotMachine.CachedRealSymbols.Count)]);
                slots[4].SetType(VegasSevenSlotMachine.CachedRealSymbols[Random.Range(0, VegasSevenSlotMachine.CachedRealSymbols.Count)]);
            }
            else
            {
        ...
        {
            if (slots[2].type == VegasSevenSlotType.Empty)
            {
                slots[1].SetType(VegasSevenSlotMachine.CachedRealSymbols[Random.Range(0, VegasSevenSlotMachine.CachedRealSymbols.Count)]);
                slots[5].SetType(VegasSevenSlotMachine.CachedRealSymbols[Random.Range(0, VegasSevenSlotMachine.CachedRealSymbols.Count)]);
            }
            else
            {
    ...
    public VegasSevenSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == VegasSevenSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? VegasSevenGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? VegasSevenGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private void OnClampComplete()
    {
        if (_clampedDown)
        {
            ...
            //var topSlot = VegasSevenSlotMachine.GetResourceById("Empty");
            //slots[0].SetType(topSlot.Value);
            ////slots[slots.Count - 1].SetType(topSlot.Value);

            //Debug.Log("Reel " + this._index + " Clamped Down");
        }
        VegasSevenUIManager.Instance.PlaySound("ReelStop");
        OnSpinComplete?.Invoke(this._index);
    }

    private VegasSevenResource GetNextGeneratedSymbol()
    {
        if (_nextIsRealSymbol)
        {
            _nextIsRealSymbol = false;
            var real = VegasSevenSlotMachine.CachedRealSymbols;
            return real[Random.Range(0, real.Count)];
        }
        else
        {
            _nextIsRealSymbol = true;
            return VegasSevenSlotMachine.CachedEmptySymbol.Value;
        }
    }
    ...
    private void Update()
    {
        ...
        //if (_increaseSpeed || !VegasSevenSlotMachine.Instance.isResultReceived)
        if (!canStopReel)
        {
