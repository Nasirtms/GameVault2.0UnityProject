...
public class UltimateFireLinkRiverWalkMiniGameReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkRiverWalkMiniGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkRiverWalkMiniGameSlotScript> slots;
    ...
    private UltimateFireLinkRiverWalkMiniGameSpinDirection currentSpinDirection = UltimateFireLinkRiverWalkMiniGameSpinDirection.Down;
    ...
    {
        var sphereRes = UltimateFireLinkRiverWalkMiniGameSlotMachine.GetResourceById("sphere");
        var emptyRes = UltimateFireLinkRiverWalkMiniGameSlotMachine.GetResourceById("empty");
    ...
    {
        settings = UltimateFireLinkRiverWalkMiniGameSlotMachine.Instance.settings;

        if (slots == null) slots = new List<UltimateFireLinkRiverWalkMiniGameSlotScript>(4);
        if (slots.Count != 4)
        {
            slots.Clear();
            slots.AddRange(GetComponentsInChildren<UltimateFireLinkRiverWalkMiniGameSlotScript>(true));
        }
    ...
    public void SetSpinDirection(UltimateFireLinkRiverWalkMiniGameSpinDirection direction) => currentSpinDirection = direction;
    ...
    private IEnumerator SmoothWindUp()
    {
        ...
        float sign = (currentSpinDirection == UltimateFireLinkRiverWalkMiniGameSpinDirection.Up) ? -1f : 1f;
    ...
    {
        float dir = (currentSpinDirection == UltimateFireLinkRiverWalkMiniGameSpinDirection.Up) ? 1f : -1f;
    ...
    {
        if (reelIndex >= UltimateFireLinkRiverWalkMiniGameSlotMachine.Instance.spinSymbolMatrix.Count)
            return;

        finalResultSymbols = UltimateFireLinkRiverWalkMiniGameSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            var res = UltimateFireLinkRiverWalkMiniGameSlotMachine.GetResourceById(symbolData.id);
            if (!res.HasValue) continue;

            UltimateFireLinkRiverWalkMiniGameSlotMachine.Instance.isResultReceived = true;

            var slot = slots[row + 1];

            if (res.Value.slotType == UltimateFireLinkRiverWalkMiniGameSlotType.Mini)
