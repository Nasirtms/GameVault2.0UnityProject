...
public class UltimateFireLinkRiverWalkSlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public UltimateFireLinkRiverWalkSlotType slotType;
    [HideInInspector] public UltimateFireLinkRiverWalkSlotResource currentResource;
    ...
    {
        var allResources = UltimateFireLinkRiverWalkSlotMachine.Instance.settings.slotResources;

        if (allResources == null || allResources.Count == 0)
            return;

        List<UltimateFireLinkRiverWalkSlotResource> filtered = new List<UltimateFireLinkRiverWalkSlotResource>();

        foreach (var r in allResources)
        {
            if (r.slotType != UltimateFireLinkRiverWalkSlotType.Freegames &&
                r.slotType != UltimateFireLinkRiverWalkSlotType.Minor &&
                r.slotType != UltimateFireLinkRiverWalkSlotType.Major &&
                r.slotType != UltimateFireLinkRiverWalkSlotType.Mega &&
                r.slotType != UltimateFireLinkRiverWalkSlotType.Mini)
            {
                filtered.Add(r);
            }
        }
    ...
    public void SetType(UltimateFireLinkRiverWalkSlotResource newType)
    {
