...
public class UltimateFireLinkRiverWalkReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkRiverWalkGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkRiverWalkSlotScript> slots;
    ...
    // Direction control
    private UltimateFireLinkRiverWalkSpinDirection currentSpinDirection = UltimateFireLinkRiverWalkSpinDirection.Down;
    ...
    {
        settings = UltimateFireLinkRiverWalkSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<UltimateFireLinkRiverWalkSlotScript>(GetComponentsInChildren<UltimateFireLinkRiverWalkSlotScript>());
        }

        foreach (UltimateFireLinkRiverWalkSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= UltimateFireLinkRiverWalkSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = UltimateFireLinkRiverWalkSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = UltimateFireLinkRiverWalkSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                UltimateFireLinkRiverWalkSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(UltimateFireLinkRiverWalkSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == UltimateFireLinkRiverWalkSpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= UltimateFireLinkRiverWalkSlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = UltimateFireLinkRiverWalkSlotMachine.GetResourceById("wild");
