...
public class UltimateFireLinkRiverWalkSlotMachine : BaseSlotMachine
{
    #region Variables

    public static UltimateFireLinkRiverWalkSlotMachine Instance;

    [Header("Machine References")]
    public UltimateFireLinkRiverWalkGameSettings settings;
    public List<UltimateFireLinkRiverWalkReelScript> reels;
    [SerializeField] private UltimateFireLinkRiverWalkBetController betController;
    ...
    private void OnSpinResultReceived(BaseSpinResult result)
    {
        ...
        UltimateFireLinkRiverWalkUIManager.Instance.SetStopInteractable(true);
    }
    ...
    private IEnumerator StartSpin()
    {
        ...
        if (!isFreeGame || firstFreeSpin)
        {
            ...
            UltimateFireLinkRiverWalkUIManager.Instance.UpdateWinAmount(0f);
            freeSpinWinAmount = 0;
            winAmount = 0f;
        }

        UltimateFireLinkRiverWalkUIManager.Instance.PlaySpinMusic("Spin");
        ...
        UltimateFireLinkRiverWalkPaylineController.Instance.ClearPaylineData();
        UltimateFireLinkRiverWalkPaylineController.Instance.StopPaylineDisplay();
        UltimateFireLinkRiverWalkUIManager.Instance.winAnimationCompleted = true;
        UltimateFireLinkRiverWalkUIManager.Instance.SetStopInteractable(false);


        if (settings.spinSettings.startSpin == UltimateFireLinkRiverWalkSpinMode.SpinAll)
        {
            for (int i = 0; i < reels.Count; i++)
            {
                UltimateFireLinkRiverWalkReelScript reel = reels[i];
    ...
    private IEnumerator WaitUntilResultAndThenStop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            else if (UltimateFireLinkRiverWalkAutoSpinController.isAutoSpinning)
            {
                UltimateFireLinkRiverWalkUIManager.Instance.CancelAutoSpin();
            }
            else
            {
                UltimateFireLinkRiverWalkUIManager.Instance.UpdateButtons("Idle");
            }
            UltimateFireLinkRiverWalkUIManager.Instance.StopSpinMusic("Spin");
            isSpinAgain = true;
    ...
    public void Stop()
    {
        ...
        if (currentSpinResult == null || currentSpinResult.reels == null || currentSpinResult.reels.Count == 0)
        {
            ...
            if (!UltimateFireLinkRiverWalkAutoSpinController.isAutoSpinning && !isFreeGame)
            {
                UltimateFireLinkRiverWalkUIManager.Instance.UpdateButtons("Idle");
            }
    ...
    private IEnumerator StopReelsWithResultRoutine()
    {
        ...
        if (settings.spinSettings.endSpin == UltimateFireLinkRiverWalkSpinMode.SpinAll)
        {
            for (int i = 0; i < reels.Count; i++)
            {
    ...
    private void ProcessSpinResult()
    {
        ...
        if (isFreeGame && winAmount > 0)
        {
            ...
            UltimateFireLinkRiverWalkUIManager.Instance.UpdateWinAmount(winAmount, true);
        }
        else if (winAmount > 0)
        {
        ...
        if ((currentSpinResult.paylineWins != null && currentSpinResult.paylineWins.Count > 0) || scatterCount >= 1)
        {
            if (currentSpinResult.paylineWins != null && currentSpinResult.paylineWins.Count > 0)
            {
                ...
                {
                    UltimateFireLinkRiverWalkPaylineResult result = new UltimateFireLinkRiverWalkPaylineResult(payline.paylineIndex, payline.count, payline.winAmount, payline.symbol);
                    UltimateFireLinkRiverWalkPaylineController.Instance.AddPaylineData(result);
        ...
        if (winAmount > 0)
        {
            UltimateFireLinkRiverWalkUIManager.Instance.PlaySound("Win");
        }
        // Spin complete
        isSpinAgain = true;
        InSpin = false;
        ...
        if(isBonusGame)
        {
            UltimateFireLinkRiverWalkUIManager.Instance.spinButton.GetButtonComponent().interactable = false;
            UltimateFireLinkRiverWalkUIManager.Instance.autoButton.GetButtonComponent().interactable = false;
        }
    ...
    private void SetReelDirection(UltimateFireLinkRiverWalkReelScript reel)
    {
        UltimateFireLinkRiverWalkSpinDirection direction = settings.spinSettings.spinDirection;

        // If random direction, choose randomly for each reel
        if (direction == UltimateFireLinkRiverWalkSpinDirection.Random)
        {
            direction = Random.value > 0.5f ? UltimateFireLinkRiverWalkSpinDirection.Up : UltimateFireLinkRiverWalkSpinDirection.Down;
        }
    ...
    {
        foreach (UltimateFireLinkRiverWalkReelScript reel in reels)
        {
            if (reel != null)
            {
                reel.ForceClampToTop();
    ...
    {
        UltimateFireLinkRiverWalkPaylineController.Instance.StartPayline(scatterCount);
    }
    ...
    public static UltimateFireLinkRiverWalkSlotResource? GetResourceById(string id)
    {
