...
public class BuffaloXtraReelPowerReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private BuffaloXtraReelPowerGameSettings settings;

    [Header("References")]
    public List<BuffaloXtraReelPowerSlotScript> slots;
    ...
    // Direction control
    private BuffaloXtraReelPowerSpinDirection currentSpinDirection = BuffaloXtraReelPowerSpinDirection.Down;
    ...
    {
        settings = BuffaloXtraReelPowerSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 7)
        {
            slots.Clear();
            slots = new List<BuffaloXtraReelPowerSlotScript>(GetComponentsInChildren<BuffaloXtraReelPowerSlotScript>());
        }

        foreach (BuffaloXtraReelPowerSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= BuffaloXtraReelPowerSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = BuffaloXtraReelPowerSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = BuffaloXtraReelPowerSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                BuffaloXtraReelPowerSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(BuffaloXtraReelPowerSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == BuffaloXtraReelPowerSpinDirection.Up)
        {
