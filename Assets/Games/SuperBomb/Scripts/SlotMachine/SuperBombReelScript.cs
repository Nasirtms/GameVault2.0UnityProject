...
public class SuperBombReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<SuperBombSlotScript> slots;
    private RectTransform _rectTransform;
    private SuperBombSpinSettings _spinSettings;
    ...
    // Events
    public delegate void SuperBombReelEvents(int index);
    public static event SuperBombReelEvents OnSpinStart;
    public static event SuperBombReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<SuperBombSlotScript>(GetComponentsInChildren<SuperBombSlotScript>());
        }
        ...
        _spinSettings = SuperBombSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        SuperBombSlotMachine.Instance.isResultReceived = false;
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = SuperBombSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = SuperBombSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
    ...
    public SuperBombSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == SuperBombSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? SuperBombGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? SuperBombGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
        ...
        if (currentPos.y <= _spinSettings.bottomBoundary)
        {
            ...
            if (_inSpin && allowSymbolChanges && finalResultSymbols == null)
            {
                ...
                if (_index == 0 || _index == 4)
                {
                    ...
                    while (attempts < maxAttempts && slots[0].type == SuperBombSlotType.Wild);
