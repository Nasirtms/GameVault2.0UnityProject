...
[System.Serializable]
public class ComeOnCashPaylineData
{
    public int paylineIndex;
}

[System.Serializable]
public class ComeOnCashPaylineEntry
{
    private ComeOnCashPaylineData payline;
    public ComeOnCashPaylineEntry(ComeOnCashPaylineData payline) { this.payline = payline; }
    public ComeOnCashPaylineData PaylineData => payline;
}

[System.Serializable]
public class ComeOnCashPaylineResult
{
    public int paylineNumber;
    public ComeOnCashPaylineResult(int paylineNumber) { this.paylineNumber = paylineNumber; }
}

public class ComeOnCashPaylineController : MonoBehaviour
{
    #region Variables

    public static ComeOnCashPaylineController Instance;

    [Header("Paylines")]
    [SerializeField] private List<ComeOnCashPaylineData> paylines;
    ...
    private List<ComeOnCashPaylineResult> spinResults = new();
    ...
    {
        for(int reel = 0; reel < ComeOnCashSlotMachine.Instance.reels.Count; reel++)
        {
            var slot = ComeOnCashSlotMachine.Instance.reels[reel].slots[1];
            if (slot == null) continue;

            if(!ComeOnCashSlotMachine.Instance.canReel1spin && reel == 0)
            {
                continue;
            }

            if(!ComeOnCashSlotMachine.Instance.canReel2spin && reel == 1)
            {
                continue;
            }

            if(!ComeOnCashSlotMachine.Instance.canReel3spin && reel == 2)
            {
                continue;
            }
        ...
        ComeOnCashSlotMachine.Instance.isPaylineCompleted = true;
    }
    ...
    private void StopPaylineSlots()
    {
        ...
        ComeOnCashUIManager.Instance.reel1PaylineBg.SetActive(false);
        if(ComeOnCashUIManager.Instance.reel2PaylineBg.activeSelf) ComeOnCashUIManager.Instance.reel2PaylineBg.SetActive(false);
        if(ComeOnCashUIManager.Instance.reel3PaylineBg.activeSelf) ComeOnCashUIManager.Instance.reel3PaylineBg.SetActive(false);
    }
}
