...
[RequireComponent(typeof(ComeOnCashBetController))]
[RequireComponent(typeof(ComeOnCashRulesPopupController))]
[RequireComponent(typeof(ComeOnCashAutoSpinController))]
public class ComeOnCashUIManager : GameBetServices
{
    #region Variables

    public static ComeOnCashUIManager Instance;
    ...
    [Header("Spin Buttons")]
    [SerializeField] public ComeOnCashUIButtonController spinButton;
    [SerializeField] private ComeOnCashUIButtonController stopButton;
    ...
    // Scripts
    public ComeOnCashBetController betController;
    private ComeOnCashRulesPopupController rulesPopupController;
    private ComeOnCashAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<ComeOnCashBetController>();
        rulesPopupController = GetComponent<ComeOnCashRulesPopupController>();
        autoSpinController = GetComponent<ComeOnCashAutoSpinController>();
    ...
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsSoundMute())
            ComeOnCashSoundManager.Instance.PlaySFX(soundName);
    }

    public void PlayMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsMusicMute())
            ComeOnCashSoundManager.Instance.PlayMusic(soundName);
    }
    ...
    public void StopMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsMusicMute())
            ComeOnCashSoundManager.Instance.StopMusic(soundName);
    }
    public void PlaySpinMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsSoundMute())
            ComeOnCashSoundManager.Instance.PlaySpinMusic(soundName);
    }
    public void StopSpinMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsSoundMute())
            ComeOnCashSoundManager.Instance.StopSpinMusic(soundName);
    }
    private void PlayWinMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsSoundMute())
            ComeOnCashSoundManager.Instance.PlayWinMusic(soundName);
    }
    ...
    private void StopWinMusic(string soundName)
    {
        if (soundName == null || ComeOnCashSoundManager.Instance == null) return;
        if (!ComeOnCashSoundManager.Instance.IsSoundMute())
            ComeOnCashSoundManager.Instance.StopWinMusic(soundName);
    }
    public void SoundActive(bool soundActive)
    {
        ComeOnCashSoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        ComeOnCashSoundManager.Instance.MuteMusic(!musicActive);
    ...
    {
        if(ComeOnCashSlotMachine.Instance.isFreeGameReady) return;
        PlaySpinMusic("Spin");
    ...
    private void OnClickStop()
    {
        ...
        ComeOnCashSlotMachine.Instance.isStopBtnPressed = true;
        ComeOnCashSlotMachine.Instance.StopWithResult();
        
        if (ComeOnCashAutoSpinController.isAutoSpinning)
        {
    ...
    private IEnumerator AnimateToValue(float target, float duration, TMP_Text textToAnimate)
    {
        ...
        Debug.Log("trying to complete bonus 8 " + winAnimationCompleted);
    }
    ...
    {
        if(ComeOnCashSlotMachine.Instance.isBonusGameEnding)
        {
            yield return new WaitUntil(() => ComeOnCashFreeGameTransitionController.Instance.canEndBonusGame);
        }
        ...
        winAnimationCompleted = true;
        Debug.Log("trying to complete bonus 7 " +winAnimationCompleted);
        if (ComeOnCashSlotMachine.Instance.isFreeGameReady)
        {
            //UpdateButtons("Transition");
        }
        else if (ComeOnCashAutoSpinController.isAutoSpinning)
        {
            UpdateButtons("Spin");
        }
        else
        ...
        if(ComeOnCashSlotMachine.Instance.isBonusGameReady)
        {
            UpdateButtons("bonusStart");
        }
        StopCoroutine(winCoroutine);
    }
