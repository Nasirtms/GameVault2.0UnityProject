...
public class GoldenWheelReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<GoldenWheelSlotScript> slots;
    private RectTransform _rectTransform;
    private GoldenWheelSpinSettings _spinSettings;
    ...
    // Events
    public delegate void GoldenWheelReelEvents(int index);
    public static event GoldenWheelReelEvents OnSpinStart;
    public static event GoldenWheelReelEvents OnSpinComplete;
    ...
    private void Start()
    {
        if (GoldenWheelSlotMachine.Instance != null)
            GoldenWheelSlotMachine.Instance.StopReelProcess += HandleStart;
    }
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<GoldenWheelSlotScript>(GetComponentsInChildren<GoldenWheelSlotScript>());
        ...
        _spinSettings = GoldenWheelSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        GoldenWheelSlotMachine.Instance.isResultReceived = false;
    }
    ...
    {
        if (reelIndex >= GoldenWheelSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = GoldenWheelSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 1; rowIndex++)
        {
            ...
            var res = GoldenWheelSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
    ...
    public GoldenWheelSlotType GetSlotType(int index)
    {
    ...
    {
        slots[0].SetType(GoldenWheelSlotMachine.CachedRealSymbols[Random.Range(0, GoldenWheelSlotMachine.CachedRealSymbols.Count)]);
        ...
        if (_spinSettings.endSpin == GoldenWheelSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? GoldenWheelGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? GoldenWheelGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private void OnClampComplete()
    {
        ...
        GoldenWheelUIManager.Instance.PlaySound("ReelStop");
        OnSpinComplete?.Invoke(this._index);
    }

    private GoldenWheelSlotResource GetNextGeneratedSymbol()
    {
        _nextIsRealSymbol = false;
        var real = GoldenWheelSlotMachine.CachedRealSymbols;
