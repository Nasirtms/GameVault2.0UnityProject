...
public class GoldenWheelSlotScript : MonoBehaviour
{
    #region Variables
    ...
    // Slot Details
    private int index;
    [HideInInspector] public GoldenWheelSlotType type;
    public GoldenWheelSlotResource currentResource;
    [HideInInspector] public bool isResultSet = false;

    // Components
    private Image icon;
    private Image paylineIcon;
    private GoldenWheelReelScript _parent;
    ...
    public void Initialize(GoldenWheelReelScript parentReel, int index)
    {
        this.index = index;
        this._parent = parentReel;
        _rectTransform = GetComponent<RectTransform>();
        icon = transform.GetChild(0).GetComponent<Image>();
        paylineIcon = transform.GetChild(1).GetComponent<Image>();
        GetRandom();
    }
    ...
    public void SetType(GoldenWheelSlotResource newType)
    {
        currentResource = newType;
        icon.sprite = newType.icon;
        paylineIcon.sprite = newType.paylineIcon;
        type = newType.type;

        uiShiny = paylineIcon.gameObject.GetComponent<UIShiny>();
    }

    public void GetRandom()
    {
        var random = GoldenWheelSlotMachine.CachedRealSymbols[
            Random.Range(0, GoldenWheelSlotMachine.CachedRealSymbols.Count)];
        SetType(random);
    }
    ...
    {
        if(type == GoldenWheelSlotType.Empty)
        {
            return;
        }
        var anim = transform.GetComponent<Animator>();
        anim.SetBool("slot", true);
    }

    public void DeAnimateSlots()
    {
        var anim = transform.GetComponent<Animator>();
        anim.SetBool("slot", false);
    }
}
