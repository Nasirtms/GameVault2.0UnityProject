...
[System.Serializable]
public class GoldenWheelPaylineData
{
    public int paylineIndex;
}

[System.Serializable]
public class GoldenWheelPaylineEntry
{
    private GoldenWheelPaylineData payline;
    public GoldenWheelPaylineEntry(GoldenWheelPaylineData payline) { this.payline = payline; }
    public GoldenWheelPaylineData PaylineData => payline;
}

[System.Serializable]
public class GoldenWheelPaylineResult
{
    public int paylineNumber;
    public GoldenWheelPaylineResult(int paylineNumber) { this.paylineNumber = paylineNumber; }
}

public class GoldenWheelPaylineController : MonoBehaviour
{
    #region Variables

    public static GoldenWheelPaylineController Instance;

    [Header("Paylines")]
    [SerializeField] private List<GoldenWheelPaylineData> paylines;
    ...
    private List<GoldenWheelPaylineResult> spinResults = new();
    ...
    {
        for(int reel = 0; reel < GoldenWheelSlotMachine.Instance.reels.Count; reel++)
        {
            var slot = GoldenWheelSlotMachine.Instance.reels[reel].slots[1];
            if (slot == null) continue;

            if(!GoldenWheelSlotMachine.Instance.canReel2spin && reel == 1)
            {
                continue;
            }

            if(!GoldenWheelSlotMachine.Instance.canReel3spin && reel == 2)
            {
                continue;
            }
        ...
        GoldenWheelSlotMachine.Instance.isPaylineCompleted = true;
    }
    ...
    private void StopPaylineSlots()
    {
        ...
        GoldenWheelUIManager.Instance.reel1PaylineBg.SetActive(false);
        if(GoldenWheelUIManager.Instance.reel2PaylineBg.activeSelf) GoldenWheelUIManager.Instance.reel2PaylineBg.SetActive(false);
        if(GoldenWheelUIManager.Instance.reel3PaylineBg.activeSelf) GoldenWheelUIManager.Instance.reel3PaylineBg.SetActive(false);
    }
}
