...
public class BiggerBassBonanzaReelScript : MonoBehaviour
{
    #region Variables

    [Header("Reel Reference")]
    private BiggerBassBonanzaGameSettings settings;
    public List<BiggerBassBonanzaSlotScript> slots;
    ...
    private BiggerBassBonanzaSpinDirection currentSpinDirection = BiggerBassBonanzaSpinDirection.Down;

    // Result Data
    private List<BiggerBassBonanzaSymbolData> finalResultSymbols;
    ...
    {
        settings = BiggerBassBonanzaSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<BiggerBassBonanzaSlotScript>(GetComponentsInChildren<BiggerBassBonanzaSlotScript>());
        }

        foreach (BiggerBassBonanzaSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= BiggerBassBonanzaSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            Debug.LogError($"No spin data for reel {reelIndex}!");
            return;
        }

        var symbols = BiggerBassBonanzaSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = BiggerBassBonanzaSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                BiggerBassBonanzaSlotMachine.Instance.isResultReceived = true;

                var slot = slots[rowIndex + 1]; // Make sure slots[1], [2], [3] are the visible ones

                if (BiggerBassBonanzaSlotMachine.Instance.isFishSlot(res.Value.slotType))
                {
                    slot.UpdateFishAmount(symbolData.fishAmount);
                }
                else
                {
                    slot.UpdateFishAmount(0f);
                }
    ...
    public void SetSpinDirection(BiggerBassBonanzaSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == BiggerBassBonanzaSpinDirection.Up)
        {
