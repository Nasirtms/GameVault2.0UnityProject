...
public class PiratesOfTheCaribbeanReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private PiratesOfTheCaribbeanGameSettings settings;

    [Header("References")]
    public List<PiratesOfTheCaribbeanSlotScript> slots;
    ...
    // Direction control
    private PiratesOfTheCaribbeanSpinDirection currentSpinDirection = PiratesOfTheCaribbeanSpinDirection.Down;
    ...
    {
        settings = PiratesOfTheCaribbeanSlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<PiratesOfTheCaribbeanSlotScript>(GetComponentsInChildren<PiratesOfTheCaribbeanSlotScript>());
        }

        foreach (PiratesOfTheCaribbeanSlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = PiratesOfTheCaribbeanSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = PiratesOfTheCaribbeanSlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                PiratesOfTheCaribbeanSlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(PiratesOfTheCaribbeanSpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == PiratesOfTheCaribbeanSpinDirection.Up)
        {
