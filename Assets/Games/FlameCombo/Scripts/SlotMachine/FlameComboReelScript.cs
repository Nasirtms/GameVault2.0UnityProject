...
public class FlameComboReelScript : MonoBehaviour
{
    #region Variables

    [Header("Slot Machine")]
    [SerializeField] private VerticalLayoutGroup verticalLayout;
    public List<FlameComboSlotScript> slots;

    private RectTransform _rectTransform;
    private FlameComboSpinSettings _spinSettings;
    private Vector2 _targetPos;

    public delegate void FlameComboReelEvents(int index);
    public static event FlameComboReelEvents OnSpinStart;
    public static event FlameComboReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<FlameComboSlotScript>(GetComponentsInChildren<FlameComboSlotScript>());
        ...
        _spinSettings = FlameComboSlotMachine.Instance.settings.spinSettings;
    }
    #endregion
    ...
    {
        FlameComboSlotMachine.Instance.isResultReceived = false;
    }
    ...
    {
        if (reelIndex >= FlameComboSlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }
        var symbols = FlameComboSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = FlameComboSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
    ...
    public FlameComboSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == FlameComboSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? FlameComboGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? FlameComboGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
