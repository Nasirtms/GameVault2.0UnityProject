...
[System.Serializable]
public class CashMachinePaylineData
{
    public int paylineIndex;
}

[System.Serializable]
public class CashMachinePaylineEntry
{
    private CashMachinePaylineData payline;
    public CashMachinePaylineEntry(CashMachinePaylineData payline) { this.payline = payline; }
    public CashMachinePaylineData PaylineData => payline;
}

[System.Serializable]
public class CashMachinePaylineResult
{
    public int paylineNumber;
    public CashMachinePaylineResult(int paylineNumber) { this.paylineNumber = paylineNumber; }
}

public class CashMachinePaylineController : MonoBehaviour
{
    #region Variables

    public static CashMachinePaylineController Instance;

    [Header("Paylines")]
    [SerializeField] private List<CashMachinePaylineData> paylines;
    ...
    private List<CashMachinePaylineResult> spinResults = new();
    ...
    public void PlayPaylineSlots()
    {
        for(int reel = 0; reel < CashMachineSlotMachine.Instance.reels.Count; reel++)
        {
            var slot = CashMachineSlotMachine.Instance.reels[reel].slots[1];
            if (slot == null) continue;

            if(!CashMachineSlotMachine.Instance.canReel2spin && reel == 1)
            {
                continue;
            }

            if(!CashMachineSlotMachine.Instance.canReel3spin && reel == 2)
            {
                continue;
            }
    ...
    private void StopPaylineSlots()
    {
        ...
        CashMachineUIManager.Instance.reel1PaylineBg.SetActive(false);
        if(CashMachineUIManager.Instance.reel2PaylineBg.activeSelf) CashMachineUIManager.Instance.reel2PaylineBg.SetActive(false);
        if(CashMachineUIManager.Instance.reel3PaylineBg.activeSelf) CashMachineUIManager.Instance.reel3PaylineBg.SetActive(false);
    }
}
