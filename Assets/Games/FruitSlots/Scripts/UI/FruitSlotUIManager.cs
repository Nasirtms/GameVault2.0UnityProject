...
[RequireComponent(typeof(FruitSlotBetController))]
[RequireComponent(typeof(FruitSlotRulesPopupController))]
[RequireComponent(typeof(FruitSlotAutoSpinController))]
public class FruitSlotUIManager : GameBetServices
{   
    #region Variables
    public static FruitSlotUIManager Instance;
    ...
    [Header("Spin Buttons")]
    [SerializeField] public FruitSlotUIButtonController spinButton;
    [SerializeField] private Image autoSpinIndiactorImage;
    [SerializeField] public FruitSlotUIButtonController stopButton;
    ...
    private FruitSlotBetController betController;
    private FruitSlotRulesPopupController rulesPopupController;
    private FruitSlotAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<FruitSlotBetController>();
        rulesPopupController = GetComponent<FruitSlotRulesPopupController>();
        autoSpinController = GetComponent<FruitSlotAutoSpinController>();
    ...
    public void PlaySound(string soundName)
    {
        ...
        if (!FruitSlotSoundManager.Instance.IsSoundMute())
            FruitSlotSoundManager.Instance.PlaySFX(soundName);
    }
    ...
    private void PlayWinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!FruitSlotSoundManager.Instance.IsSoundMute())
            FruitSlotSoundManager.Instance.PlayWinMusic(soundName);
    }
    ...
    public void PlayMusic(string soundName)
    {
        ...
        if (!FruitSlotSoundManager.Instance.IsMusicMute())
            FruitSlotSoundManager.Instance.PlayMusic(soundName);
    }
    ...
    private void StopWinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!FruitSlotSoundManager.Instance.IsSoundMute())
            FruitSlotSoundManager.Instance.StopWinMusic(soundName);
    }
    private void SoundActive(bool soundActive)
    {
        FruitSlotSoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        FruitSlotSoundManager.Instance.MuteMusic(!musicActive);
    ...
    {
        if (FruitSlotMachine.Instance.InSpin) return;

        if(FruitSlotMachine.Instance.isFreeGameReady) return;
    ...
    private void OnClickStop()
    {
        ...
        FruitSlotMachine.Instance.isStopBtnPressed = true;

        if (FruitSlotAutoSpinController.isAutoSpinning)
            autoSpinController.CancelAutoSpin();

        FruitSlotMachine.Instance.StopWithResult();
        FruitSlotMachine.Instance.InvokeStop();
    }
    ...
    public void OnHoldSpin()
    {
        ...
        if (FruitSlotMachine.Instance.InSpin || FruitSlotAutoSpinController.isAutoSpinning)
        {
            return;
        }
        UpdateButtons("Spin");
    ...
    #endregion
    public void SetSpinInProgress(bool isSpinning)
    {
        autoSpin = autoSpinController.IsAutoRunning;
        increaseBetButton.interactable = !isSpinning;
        decreaseBetButton.interactable = !isSpinning;
        exitGameButton.interactable = !isSpinning;
        maxBetButton.interactable = !isSpinning;
        openRulesButton.interactable = !isSpinning;
        autoSpinIndiactorImage.enabled = !isSpinning;

        spinButton.GetButtonComponent().interactable = !isSpinning && !autoSpin;

        if (FruitSlotMachine.Instance.winAmount <= 0)
        {
            FruitSlotMachine.Instance.isSpinAgain = true;
        }
    }
    ...
    public void TextAnimation(float target, float duration, TMP_Text textToAnimate)
    {
        ...
        //freeGameWinAmount += FruitSlotMachine.Instance.winAmount;
