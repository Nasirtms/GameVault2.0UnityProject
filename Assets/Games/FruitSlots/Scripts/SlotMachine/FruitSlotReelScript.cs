...
public class FruitSlotReelScript : MonoBehaviour
{
    public delegate void FruitSlotReelEvents(int index);
    public static event FruitSlotReelEvents OnSpinStart;
    public static event FruitSlotReelEvents OnSpinComplete;

    [SerializeField] private VerticalLayoutGroup verticalLayout;
    [SerializeField] private List<FruitSlotType> slotsType = new List<FruitSlotType>();
    public List<FruitSlotScript> slots;
    ...
    private FruitSlotSpinSettings _spinSettings;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<FruitSlotScript>(GetComponentsInChildren<FruitSlotScript>());
        }
        ...
        _spinSettings = FruitSlotMachine.Instance.settings.spinSettings;
    }
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = FruitSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = FruitSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;
    ...
    {
        FruitSlotMachine.Instance.isResultReceived = false;
    ...
    public FruitSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == FruitSlotSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? FruitSlotGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? FruitSlotGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
    ...
    private void Update()
    {
        ...
        //if (_increaseSpeed || !FruitSlotMachine.Instance.isResultReceived)
        if (!canStopReel)
        {
    ...
    {
        Dictionary<string, HashSet<FruitSlotType>> reelSlotTypes = new();
        ...
        foreach (var slot in slots)
        {
            ...
            if (!reelSlotTypes.ContainsKey(reelName))
                reelSlotTypes[reelName] = new HashSet<FruitSlotType>();
