...
public class StickyPiggyAutoSpinController : MonoBehaviour
{
    #region Variables
    [Header("Settings")]
    ...
    {
        if (isAutoRunning || StickyPiggySlotMachine.Instance.InSpin)
        {
            return;
        }
    ...
    private IEnumerator AutoSpinLoop(float betAmount)
    {
        while (!cancelRequested)
        {
            ...
            if (StickyPiggyUIManager.Instance.textAnimationCoroutine != null)
                StopCoroutine(StickyPiggyUIManager.Instance.textAnimationCoroutine);

            if (StickyPiggyUIManager.Instance.winCoroutine != null)
                StopCoroutine(StickyPiggyUIManager.Instance.winCoroutine);
