...
public class StickyPiggyReelScript : MonoBehaviour
{
    #region Variables

    // Machine Variables
    [Header("Settings Reference")]
    private StickyPiggyGameSettings settings;

    [Header("References")]
    public List<StickyPiggySlotScript> slots;
    ...
    // Direction control
    private StickyPiggySpinDirection currentSpinDirection = StickyPiggySpinDirection.Down;
    ...
    {
        settings = StickyPiggySlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<StickyPiggySlotScript>(GetComponentsInChildren<StickyPiggySlotScript>());
        }

        foreach (StickyPiggySlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= StickyPiggySlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = StickyPiggySlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = StickyPiggySlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                StickyPiggySlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(StickyPiggySpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == StickyPiggySpinDirection.Up)
        {
