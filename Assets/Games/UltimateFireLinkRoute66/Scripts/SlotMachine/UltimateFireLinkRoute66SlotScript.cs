...
public class UltimateFireLinkRoute66SlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public UltimateFireLinkRoute66SlotType slotType;
    [HideInInspector] public UltimateFireLinkRoute66SlotResource currentResource;
    ...
    {
        var allResources = UltimateFireLinkRoute66SlotMachine.Instance.settings.slotResources;

        if (allResources == null || allResources.Count == 0)
            return;

        List<UltimateFireLinkRoute66SlotResource> filtered = new List<UltimateFireLinkRoute66SlotResource>();

        foreach (var r in allResources)
        {
            if (r.slotType != UltimateFireLinkRoute66SlotType.Freegames &&
                r.slotType != UltimateFireLinkRoute66SlotType.Minor &&
                r.slotType != UltimateFireLinkRoute66SlotType.Major &&
                r.slotType != UltimateFireLinkRoute66SlotType.Mega &&
                r.slotType != UltimateFireLinkRoute66SlotType.Mini)
            {
                filtered.Add(r);
            }
        }
    ...
    public void SetType(UltimateFireLinkRoute66SlotResource newType)
    {
