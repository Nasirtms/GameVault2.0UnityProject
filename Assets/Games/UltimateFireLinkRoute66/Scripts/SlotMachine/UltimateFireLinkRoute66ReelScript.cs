...
public class UltimateFireLinkRoute66ReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkRoute66GameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkRoute66SlotScript> slots;
    ...
    // Direction control
    private UltimateFireLinkRoute66SpinDirection currentSpinDirection = UltimateFireLinkRoute66SpinDirection.Down;
    ...
    {
        settings = UltimateFireLinkRoute66SlotMachine.Instance.settings;
        UpdateSlotScale(settings.slotSettings.SymbolScaleX, settings.slotSettings.SymbolScaleY);

        if (slots == null || slots.Count != 6)
        {
            slots.Clear();
            slots = new List<UltimateFireLinkRoute66SlotScript>(GetComponentsInChildren<UltimateFireLinkRoute66SlotScript>());
        }

        foreach (UltimateFireLinkRoute66SlotScript slot in slots)
        {
            slot.Initialize();
        }
    ...
    {
        if (reelIndex >= UltimateFireLinkRoute66SlotMachine.Instance.spinSymbolMatrix.Count)
        {
            return;
        }

        var symbols = UltimateFireLinkRoute66SlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 4; rowIndex++)
        {
            ...
            var res = UltimateFireLinkRoute66SlotMachine.GetResourceById(symbolData.id);

            if (res.HasValue)
            {
                UltimateFireLinkRoute66SlotMachine.Instance.isResultReceived = true;
    ...
    public void SetSpinDirection(UltimateFireLinkRoute66SpinDirection direction)
    {
    ...
    private IEnumerator StartSpinWithWindUp()
    {
        ...
        if (currentSpinDirection == UltimateFireLinkRoute66SpinDirection.Up)
        {
    ...
    {
        if (reelIndex < 0 || reelIndex >= UltimateFireLinkRoute66SlotMachine.Instance.spinSymbolMatrix.Count) return;
        ...
        var res = UltimateFireLinkRoute66SlotMachine.GetResourceById("wild");
