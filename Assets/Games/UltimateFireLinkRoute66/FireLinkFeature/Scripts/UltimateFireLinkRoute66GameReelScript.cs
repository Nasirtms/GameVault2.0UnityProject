...
public class UltimateFireLinkRoute66MiniGameReelScript : MonoBehaviour
{
    #region Variables

    [Header("Settings Reference")]
    private UltimateFireLinkRoute66MiniGameSettings settings;

    [Header("References")]
    public List<UltimateFireLinkRoute66MiniGameSlotScript> slots;
    ...
    private UltimateFireLinkRoute66MiniGameSpinDirection currentSpinDirection = UltimateFireLinkRoute66MiniGameSpinDirection.Down;
    ...
    {
        var sphereRes = UltimateFireLinkRoute66MiniGameSlotMachine.GetResourceById("sphere");
        var emptyRes = UltimateFireLinkRoute66MiniGameSlotMachine.GetResourceById("empty");
    ...
    {
        settings = UltimateFireLinkRoute66MiniGameSlotMachine.Instance.settings;

        if (slots == null) slots = new List<UltimateFireLinkRoute66MiniGameSlotScript>(4);
        if (slots.Count != 4)
        {
            slots.Clear();
            slots.AddRange(GetComponentsInChildren<UltimateFireLinkRoute66MiniGameSlotScript>(true));
        }
    ...
    public void SetSpinDirection(UltimateFireLinkRoute66MiniGameSpinDirection direction) => currentSpinDirection = direction;
    ...
    private IEnumerator SmoothWindUp()
    {
        ...
        float sign = (currentSpinDirection == UltimateFireLinkRoute66MiniGameSpinDirection.Up) ? -1f : 1f;
    ...
    {
        float dir = (currentSpinDirection == UltimateFireLinkRoute66MiniGameSpinDirection.Up) ? 1f : -1f;
    ...
    {
        if (reelIndex >= UltimateFireLinkRoute66MiniGameSlotMachine.Instance.spinSymbolMatrix.Count)
            return;

        finalResultSymbols = UltimateFireLinkRoute66MiniGameSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int row = 0; row < 3; row++)
        {
            ...
            var res = UltimateFireLinkRoute66MiniGameSlotMachine.GetResourceById(symbolData.id);
            if (!res.HasValue) continue;

            UltimateFireLinkRoute66MiniGameSlotMachine.Instance.isResultReceived = true;

            var slot = slots[row + 1];

            if (res.Value.slotType == UltimateFireLinkRoute66MiniGameSlotType.Mini)
