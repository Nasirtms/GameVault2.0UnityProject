...
public class UltimateFireLinkRoute66MiniGameManager : MonoBehaviour
{
    #region Variables
    public static UltimateFireLinkRoute66MiniGameManager Instance;
    ...
    {
        UltimateFireLinkRoute66SlotMachine.Instance.isMiniGame = true;
    ...
    private IEnumerator StartReSpinGame()
    {
        ...
        UltimateFireLinkRoute66MiniGameSlotMachine.Instance.lockedReelIndexes.Clear();
        if (UltimateFireLinkRoute66SlotMachine.Instance.miniGame1)
        {
            var mini = UltimateFireLinkRoute66MiniGameSlotMachine.Instance;

            mini.lockedReelIndexes.AddRange(UltimateFireLinkRoute66SlotMachine.Instance.miniGameLockedReels);
        ...
        Debug.Log("LockedReelIndexes : " + UltimateFireLinkRoute66MiniGameSlotMachine.Instance.lockedReelIndexes);
        UltimateFireLinkRoute66SlotMachine.Instance.BaseGameReelsObject.SetActive(false);
        UltimateFireLinkRoute66SlotMachine.Instance.MiniGameReelsObject.SetActive(true);
        ...
        UltimateFireLinkRoute66UIManager.Instance.UpdateButtons("Free Spin");
    ...
    private IEnumerator EndReSpinGame()
    {
        ...
        UltimateFireLinkRoute66UIManager.Instance.UpdateButtons("Transition End");

        PopupAnimation(miniGameWinFrame, 1f, 1f, true);

        yield return new WaitForSeconds(1f);
        UltimateFireLinkRoute66SlotMachine.Instance.MiniGameReelsObject.SetActive(false);
        UltimateFireLinkRoute66SlotMachine.Instance.BaseGameReelsObject.SetActive(true);
        ...
        //else
        //{
        UltimateFireLinkRoute66UIManager.Instance.UpdateButtons("Free Spin End");
        //}
        ResetMiniGameState();
    }
    ...
    {
        yield return new WaitForSeconds(1f);
        currentLockedCount = UltimateFireLinkRoute66MiniGameSlotMachine.Instance.lockedReelIndexes.Count;
        while (featureRunning && reSpinsLeft > 0)
        {
            if (firstReSpin)
            {
                UltimateFireLinkRoute66MiniGameSlotMachine.Instance.firstReSpin = false;
                firstReSpin = false;
            }
            else
            {
            ...
            if (UltimateFireLinkRoute66MiniGameSlotMachine.Instance.AreAllReelsLocked())
                break;

            int previousLockedCount = UltimateFireLinkRoute66MiniGameSlotMachine.Instance.lockedReelIndexes.Count;
            ...
            yield return new WaitUntil(() => UltimateFireLinkRoute66MiniGameSlotMachine.Instance.isSpinAgain);
    ...
    private void EndReSpins()
    {
        ...
        reSpinsLeft = 0;
        isMiniGame = false;
        UltimateFireLinkRoute66SlotMachine.Instance.isMiniGame = false;
    ...
    private IEnumerator CollectSpheresToTarget()
    {
        ...
        foreach (int reelIndex in reelsLocked)
        {
            ...
            if (slot.slotType != UltimateFireLinkRoute66MiniGameSlotType.Mini)
                continue;
    ...
    private void ResetMiniGameState()
    {
        ...
        foreach (var reel in UltimateFireLinkRoute66MiniGameSlotMachine.Instance.reels)
        {
