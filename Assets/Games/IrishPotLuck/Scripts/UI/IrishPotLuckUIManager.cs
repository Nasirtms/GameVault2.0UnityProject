...
[RequireComponent(typeof(IrishPotLuckBetController))]
[RequireComponent(typeof(IrishPotLuckAutoSpinController))]
public class IrishPotLuckUIManager : GameBetServices
{
    #region Variables

    public static IrishPotLuckUIManager Instance;
    [SerializeField] private IrishPotLuckRulesPopupController rulesPopupController;
    ...
    private IrishPotLuckBetController betController;
    private IrishPotLuckAutoSpinController autoSpinController;
    ...
    private void Start()
    {
        ...
        betController = GetComponent<IrishPotLuckBetController>();
        autoSpinController = GetComponent<IrishPotLuckAutoSpinController>();
    ...
    public void PlaySound(string soundName)
    {
        ...
        if (!IrishPotLuckSoundManager.Instance.IsSoundMute())
            IrishPotLuckSoundManager.Instance.PlaySFX(soundName);
    }
    public void StopCurrentSFX()
    {
        if (!IrishPotLuckSoundManager.Instance.IsSoundMute())
            IrishPotLuckSoundManager.Instance.StopSFX();
    }
    public void PlayMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!IrishPotLuckSoundManager.Instance.IsMusicMute())
            IrishPotLuckSoundManager.Instance.PlayMusic(soundName);
    }
    ...
    public void StopMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!IrishPotLuckSoundManager.Instance.IsMusicMute())
            IrishPotLuckSoundManager.Instance.StopMusic(soundName);
    }
    public void PlaySpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!IrishPotLuckSoundManager.Instance.IsSoundMute())
            IrishPotLuckSoundManager.Instance.SpinPlayMusic(soundName);
    }
    ...
    public void StopSpinMusic(string soundName)
    {
        if (string.IsNullOrEmpty(soundName)) return;
        if (!IrishPotLuckSoundManager.Instance.IsSoundMute())
            IrishPotLuckSoundManager.Instance.SpinStopMusic(soundName);
    }

    private void SoundActive(bool soundActive)
    {
        IrishPotLuckSoundManager.Instance.MuteSFX(!soundActive);
    ...
    {
        IrishPotLuckSoundManager.Instance.MuteMusic(!musicActive);
    ...
    private void OnClickStop()
    {
        ...
        StopCurrentSFX();
        //StopSpinMusic("Spin");
        IrishPotLuckSlotMachine.Instance.isStopBtnPressed = true;
        IrishPotLuckSlotMachine.Instance.StopWithResult();
        if (IrishPotLuckAutoSpinController.isAutoSpinning)
        {
