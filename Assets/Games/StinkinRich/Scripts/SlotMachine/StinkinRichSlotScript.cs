using DG.Tweening;
using System;
using System.Collections;
using System.Collections.Generic;
using TMPro;
using UnityEngine;
using Random = UnityEngine.Random;

public class StinkinRichSlotScript : MonoBehaviour
{
    #region Variables

    // Slot Resource
    [HideInInspector] public StinkinRichSlotType slotType;
    [HideInInspector] public StinkinRichSlotResource currentResource;

    // Slot Children
    [SerializeField] private GameObject[] normalSlots;
    [SerializeField] private GameObject[] freeGameSlots;
    [SerializeField] private TMP_Text multiplier;

    // Slot Animation
    private Animator slotAnimator;
    private Animator trashSlotAnimator;
    private String slotAnimationBool;
    private String freeSpinAnimationBool;

    // Active Slot Sprites
    private SpriteRenderer[] slotRenderers;

    #endregion

    #region Slot Initialization

    public void Initialize()
    {
        // Pick a NON-WILD symbol on initialization
        //var allResources = StinkinRichSlotMachine.Instance.settings.slotResources;

        //if (allResources != null && allResources.Count > 0)
        //{
        //    List<StinkinRichSlotResource> nonWild = new List<StinkinRichSlotResource>();

        //    foreach (var r in allResources)
        //    {
        //        if (r.slotType != StinkinRichSlotType.TrashForCash || r.slotType != StinkinRichSlotType.Wild || r.slotType!= StinkinRichSlotType.KeysToRiches)
        //        {
        //            nonWild.Add(r);
        //        }
        //    }

        //    if (nonWild.Count > 0)
        //    {
        //        var random = nonWild[UnityEngine.Random.Range(0, nonWild.Count)];
        //        SetType(random);
        //        return;
        //    }
        //}
        GetRandom();
    }


    public void UpdateScale(float scaleX, float scaleY)
    {
        transform.localScale = new Vector3(scaleX, scaleY, 1);
    }

    #endregion

    #region Slot Switching

    public void GetRandom(bool blur = false)
    {
        var allResources = StinkinRichSlotMachine.Instance.settings.slotResources;

        if (allResources == null || allResources.Count == 0)
            return;

        List<StinkinRichSlotResource> filtered = new List<StinkinRichSlotResource>();

        foreach (var r in allResources)
        {
            if (r.slotType != StinkinRichSlotType.TrashForCash &&
                r.slotType != StinkinRichSlotType.Wild &&
                r.slotType != StinkinRichSlotType.KeysToRiches)
            {
                filtered.Add(r);
            }
        }

        // fallback safety (important)
        if (filtered.Count == 0)
        {
            var randomFallback = allResources[Random.Range(0, allResources.Count)];
            SetType(randomFallback);
            return;
        }

        var random = filtered[Random.Range(0, filtered.Count)];
        SetType(random);
    }

    public void SetType(StinkinRichSlotResource newType)
    {
        if (StinkinRichSlotMachine.Instance.isFreeGame)
        {
            freeGameSlots[currentResource.slotTypeIndex].SetActive(false);
        }
        else
        {
            normalSlots[currentResource.slotTypeIndex].SetActive(false);
        }

        this.currentResource = newType;
        this.slotType = newType.slotType;
        this.slotAnimationBool = newType.slotAnimationBool;
        this.freeSpinAnimationBool = newType.freeSpinAnimationBool;

        if (StinkinRichSlotMachine.Instance.isFreeGame)
        {
            freeGameSlots[newType.slotTypeIndex].SetActive(true);
        }
        else
        {
            normalSlots[newType.slotTypeIndex].SetActive(true);
        }
    }

    #endregion

    #region Slot Animation

    public void PlayAnimation()
    {
        StopAnimation();
        slotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
        if (StinkinRichSlotMachine.Instance.isFreeGame)
        {
            slotAnimator.SetBool(freeSpinAnimationBool, true);
        }
        else
        {
            slotAnimator.SetBool(slotAnimationBool, true);
        }
    }

    public void StopAnimation()
    {
        if (slotAnimator != null)
        {
            if (StinkinRichSlotMachine.Instance.isFreeGame)
            {
                slotAnimator.SetBool(freeSpinAnimationBool, false);
            }
            else
            {
                slotAnimator.SetBool(slotAnimationBool, false);
            }
        }
    }

    public void PlayWildShiftAnimation()
    {
        slotAnimator = normalSlots[currentResource.slotTypeIndex].GetComponentInParent<Animator>();
        slotAnimator.SetBool("Wild-Shift", true);
    }

    #endregion

    #region Slot Layering

    public void SetSpriteToPayline()
    {
        slotRenderers = normalSlots[currentResource.slotTypeIndex].gameObject.GetComponentsInChildren<SpriteRenderer>();

        foreach (var slot in slotRenderers)
        {
            slot.sortingLayerName = "Payline Slot";
        }
    }

    public void SetSpriteToDefault()
    {
        slotRenderers = normalSlots[currentResource.slotTypeIndex].gameObject.GetComponentsInChildren<SpriteRenderer>();
        foreach (var slot in slotRenderers)
        {
            slot.sortingLayerName = "Default";
        }
    }

    //public void SetWildPosition()
    //{
    //    var wild = normalSlots[currentResource.slotTypeIndex].transform;
    //    wild.position = new Vector3(wild.position.x, 2.5f, wild.position.z);

    //    wild.DOMoveY(0, 0.4f).SetEase(Ease.OutCubic);
    //}

    public void SetSlotActive(bool isActive)
    {
        if (normalSlots == null || normalSlots.Length == 0)
            return;

        int index = currentResource.slotTypeIndex;

        if (index < 0 || index >= normalSlots.Length)
            return;

        GameObject currentSymbolObject = normalSlots[index];
        if (currentSymbolObject == null)
            return;

        var renderers = currentSymbolObject.GetComponentsInChildren<SpriteRenderer>(true);
        float targetAlpha = isActive ? 1f : 0f;

        foreach (var r in renderers)
        {
            Color c = r.color;
            c.a = targetAlpha;
            r.color = c;
        }
    }

    public void SetFreeSpinSlots(bool enable)
    {
        if (normalSlots == null || normalSlots.Length == 0 || freeGameSlots == null || freeGameSlots.Length == 0)
            return;

        int index = currentResource.slotTypeIndex;

        if (index < 0 || index >= normalSlots.Length || index >= freeGameSlots.Length)
            return;

        normalSlots[index].SetActive(!enable);
        freeGameSlots[index].SetActive(enable);
    }

    public void PlayTrashAnimations()
    {
        if(StinkinRichSlotMachine.Instance.isFreeGame)
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashBool, true);
        }
        else
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashBool, true);
        }
    }

    public void showMultiplier()
    {
        if (StinkinRichSlotMachine.Instance.isFreeGame)
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashOpenBool, true);

            int m = StinkinRichSlotMachine.Instance.trashMultiplier;
            multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
        }
        else
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashOpenBool, true);

            int m = StinkinRichSlotMachine.Instance.trashMultiplier;
            multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
        }
    }

    public void ShowNonMultipliers()
    {
        if (StinkinRichSlotMachine.Instance.isFreeGame)
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashOpenBool, true);

            if (StinkinRichPaylineController.Instance.countOfNonTrashSlotsAnimated == 1)
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier1;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
            else if (StinkinRichPaylineController.Instance.countOfNonTrashSlotsAnimated == 2)
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier2;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
            else
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier1;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
        }
        else
        {
            trashSlotAnimator = normalSlots[currentResource.slotTypeIndex].transform.GetComponentInParent<Animator>();
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashOpenBool, true);

            if (StinkinRichPaylineController.Instance.countOfNonTrashSlotsAnimated == 1)
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier1;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
            else if (StinkinRichPaylineController.Instance.countOfNonTrashSlotsAnimated == 2)
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier2;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
            else
            {
                int m = StinkinRichSlotMachine.Instance.trashNotMultiplier1;
                multiplier.text = "<sprite index= " + m + ">" + "<sprite index=10>";
            }
        }
    }

    public void StopTrashSlotAnimations()
    {
        if (trashSlotAnimator != null)
        {
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.normalTrashOpenBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashBool, false);
            trashSlotAnimator.SetBool(StinkinRichUIManager.Instance.freeSpinTrashOpenBool, false);
        }
    }

    #endregion
}
