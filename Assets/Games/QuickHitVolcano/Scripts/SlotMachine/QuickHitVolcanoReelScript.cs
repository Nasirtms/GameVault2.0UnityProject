...
public class QuickHitVolcanoReelScript : MonoBehaviour
{
    #region Variables
    ...
    public List<QuickHitVolcanoSlotScript> slots;
    private RectTransform _rectTransform;
    private QuickHitVolcanoSpinSettings _spinSettings;
    ...
    //Events
    //public delegate void QuickHitVolcanoReelEvents(int index);
    //public static event QuickHitVolcanoReelEvents OnSpinStart;
    //public static event QuickHitVolcanoReelEvents OnSpinComplete;
    ...
    public void Initialize(int index)
    {
        ...
        {
            slots = new List<QuickHitVolcanoSlotScript>(GetComponentsInChildren<QuickHitVolcanoSlotScript>());
        }
        ...
        _spinSettings = QuickHitVolcanoSlotMachine.Instance.settings.spinSettings;
    }
    ...
    {
        QuickHitVolcanoSlotMachine.Instance.isResultReceived = false;
    ...
    public void ApplyFinalResult(int reelIndex)
    {
        ...
        var symbols = QuickHitVolcanoSlotMachine.Instance.spinSymbolMatrix[reelIndex];
        ...
        for (int rowIndex = 0; rowIndex < 3; rowIndex++)
        {
            ...
            var res = QuickHitVolcanoSlotMachine.GetResourceById(symbolData.id);
            int row = rowIndex;

            if (res.HasValue)
            {
                //QuickHitVolcanoSlotMachine.Instance.isResultReceived = true;
    ...
    public QuickHitVolcanoSlotType GetSlotType(int index)
    {
    ...
    public void Spin(float delay, float acceleration, float speed)
    {
        ...
        if (_spinSettings.endSpin == QuickHitVolcanoSpinType.All)
        {
            _delayAmount = 0f;
        }
        else
        ...
        _acceleration = acceleration <= 0 ? QuickHitVolcanoGameExtension.GetRandomValue(_spinSettings.acceleration) : acceleration;
        _timeCounter = 0f;
        _currentSpeed = speed <= 0f ? QuickHitVolcanoGameExtension.GetRandomValue(_spinSettings.startSpeed) : speed;
        _yOffset = 0f;
        _increaseSpeed = true;
        _inClamp = false;
        _inSpin = true;
